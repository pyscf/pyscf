...
def siesta_ion_interp(rr, sp2ion, fj=1):
  ...
  for sp in range(nsp):
    for mu in range(len(sp2ion[sp]["paos"]["orbital"])):
      npts,j,h = len(sp2ion[sp]["paos"]["data"][mu]),sp2ion[sp]["paos"]["orbital"][mu]["l"],sp2ion[sp]["paos"]["delta"][mu]
      yy = numpy.array([sp2ion[sp]["paos"]["data"][mu][ir][1] for ir in range(npts)], dtype='float64')
