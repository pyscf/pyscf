...
! the electron-1 operator (between (ij|) should be *anti-Hermitian*,
! the electron-2 operator (between |kl)) should be *Hermitian*.
!
!  nr_..._dm2  vj/vk is based on the density of electron 2
!  nr_..._dm12 vj/vk is based on the density of electron 1 and 2
!
! requirement
!  Density matrix should be *Hermitian*

subroutine nr_has_hs_dm2_O0(intor, fscreen, dm, vj, vk, &
                            ndim, nset, nset_dm, pair_kl, ao_loc, &
                            atm, natm, bas, nbas, env, opt)
  call nr_hs_hs_dm2_O0(intor, fscreen, dm, vj, vk, &
                       ndim, nset, nset_dm, pair_kl, ao_loc, &
                       atm, natm, bas, nbas, env, opt)
end subroutine nr_has_hs_dm2_O0

subroutine nr_has_hs_dm2_O1(intor, fscreen, dm, vj, vk, &
                            ndim, nset, nset_dm, pair_kl, ao_loc, &
                            atm, natm, bas, nbas, env, opt)
  call nr_hs_hs_dm2_O1(intor, fscreen, dm, vj, vk, &
                       ndim, nset, nset_dm, pair_kl, ao_loc, &
                       atm, natm, bas, nbas, env, opt)
end subroutine nr_has_hs_dm2_O1

subroutine nr_has_hs_dm2_O2(intor, fscreen, dm, vj, vk, &
                            ndim, nset, nset_dm, pair_kl, ao_loc, &
                            atm, natm, bas, nbas, env, opt)
  use cint_const_mod
  use cint_interface
...
end subroutine nr_has_hs_dm2_O2
! ************************************************


! ************************************************
subroutine nr_has_hs_dm12_O0(intor, fscreen, dm, vj, vk, &
                             ndim, nset, nset_dm, pair_kl, ao_loc, &
                             atm, natm, bas, nbas, env, opt)
  call nr_hs_hs_dm12_O0(intor, fscreen, dm, vj, vk, &
                        ndim, nset, nset_dm, pair_kl, ao_loc, &
                        atm, natm, bas, nbas, env, opt)
end subroutine nr_has_hs_dm12_O0

subroutine nr_has_hs_dm12_O1(intor, fscreen, dm, vj, vk, &
                             ndim, nset, nset_dm, pair_kl, ao_loc, &
                             atm, natm, bas, nbas, env, opt)
  call nr_hs_hs_dm12_O1(intor, fscreen, dm, vj, vk, &
                        ndim, nset, nset_dm, pair_kl, ao_loc, &
                        atm, natm, bas, nbas, env, opt)
end subroutine nr_has_hs_dm12_O1

subroutine nr_has_hs_dm12_O2(intor, fscreen, dm, vj, vk, &
                             ndim, nset, nset_dm, pair_kl, ao_loc, &
                             atm, natm, bas, nbas, env, opt)
  use cint_const_mod
  use cint_interface
        ...
        do is = 1, nset
          ...
          do id = 1, nset_dm
            if (0 /= do_vj(id)) then
              ...
              call HFBLK_nr_kl_O1(fjikl, dm(1,1,id), vj(1,1,is,id), ndim, tijshls, ao_loc)
            end if
            if (0 /= do_vk(id)) then
              call HFBLK_nr_il_O1(fijkl(:,:,:,:,is), dm(1,1,id), vk(1,1,is,id), ndim, shls, ao_loc)
...
end subroutine nr_has_hs_dm12_O2
