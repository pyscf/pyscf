subroutine rkb_giao_vhf_coul_O00(dm, vj, vk, ndim, atm, natm, bas, nbas, env)
!**************************************************
  use cint_const_mod
  use cint_interface
  use hf_block_mod
  ...
  call cint2e_g1(fijkl, shls, atm, natm, bas, nbas, env, 0_8)
  do is = 1, 3
    call HFBLK_ij_O0(fijkl(:,:,:,:,is), tdm(1,1), vj(1,1,is), n4c, shls, ao_loc)
    call HFBLK_kl_O0(fijkl(:,:,:,:,is), tdm(1,1), vj(1,1,is), n4c, shls, ao_loc)
    call HFBLK_il_O0(fijkl(:,:,:,:,is), tdm(1,1), vk(1,1,is), n4c, shls, ao_loc)
    call HFBLK_kj_O0(fijkl(:,:,:,:,is), tdm(1,1), vk(1,1,is), n4c, shls, ao_loc)
  end do
  call cint2e_g1spsp2(fijkl, shls, atm, natm, bas, nbas, env, 0_8)
  do is = 1, 3
    call HFBLK_ij_O0(fijkl(:,:,:,:,is), tdm(s,s), vj(1,1,is), n4c, shls, ao_loc)
    call HFBLK_kl_O0(fijkl(:,:,:,:,is), tdm(1,1), vj(s,s,is), n4c, shls, ao_loc)
    call HFBLK_il_O0(fijkl(:,:,:,:,is), tdm(1,s), vk(1,s,is), n4c, shls, ao_loc)
    call HFBLK_kj_O0(fijkl(:,:,:,:,is), tdm(s,1), vk(s,1,is), n4c, shls, ao_loc)
  end do
  call cint2e_spgsp1(fijkl, shls, atm, natm, bas, nbas, env, 0_8)
  do is = 1, 3
  ...
  call cint2e_spgsp1spsp2(fijkl, shls, atm, natm, bas, nbas, env, 0_8)
  do is = 1, 3
  ...
  deallocate (ao_loc)
  return
end subroutine rkb_giao_vhf_coul_O00

subroutine rkb_giao_vhf_coul_O01(dm, vj, vk, ndim, atm, natm, bas, nbas, env)
!**************************************************
  use cint_const_mod
  use cint_interface
  use hf_block_mod
  ...
  call cint2e_g1(fijkl, shls, atm, natm, bas, nbas, env, 0_8)
  do is = 1, 3
    call HFBLK_ij_O1(fijkl(:,:,:,:,is), tdm(1,1), vj(1,1,is), n4c, shls, ao_loc, tao)
    call HFBLK_kl_O1(fijkl(:,:,:,:,is), tdm(1,1), vj(1,1,is), n4c, shls, ao_loc, tao)
    call HFBLK_il_O1(fijkl(:,:,:,:,is), tdm(1,1), vk(1,1,is), n4c, shls, ao_loc, tao)
    call HFBLK_kj_O1(fijkl(:,:,:,:,is), tdm(1,1), vk(1,1,is), n4c, shls, ao_loc, tao)
  end do
  call cint2e_g1spsp2(fijkl, shls, atm, natm, bas, nbas, env, 0_8)
  do is = 1, 3
    call HFBLK_ij_O1(fijkl(:,:,:,:,is), tdm(s,s), vj(1,1,is), n4c, shls, ao_loc, tao)
    call HFBLK_kl_O1(fijkl(:,:,:,:,is), tdm(1,1), vj(s,s,is), n4c, shls, ao_loc, tao)
    call HFBLK_il_O1(fijkl(:,:,:,:,is), tdm(1,s), vk(1,s,is), n4c, shls, ao_loc, tao)
    call HFBLK_kj_O1(fijkl(:,:,:,:,is), tdm(s,1), vk(s,1,is), n4c, shls, ao_loc, tao)
  end do
  call cint2e_spgsp1(fijkl, shls, atm, natm, bas, nbas, env, 0_8)
  do is = 1, 3
    call HFBLK_ij_O1(fijkl(:,:,:,:,is), tdm(1,1), vj(s,s,is), n4c, shls, ao_loc, tao)
    call HFBLK_kl_O1(fijkl(:,:,:,:,is), tdm(s,s), vj(1,1,is), n4c, shls, ao_loc, tao)
  ...
  do is = 1, 3
    ...
    call HFBLK_kl_O1(fijkl(:,:,:,:,is), tdm(s,s), vj(s,s,is), n4c, shls, ao_loc, tao)
  ...
  deallocate (ao_loc)
  return
end subroutine rkb_giao_vhf_coul_O01


subroutine rkb_giao_vhf_gaunt_O00(dm, vj, vk, ndim, atm, natm, bas, nbas, env)
!**************************************************
  use cint_const_mod
  use cint_interface
  use hf_block_mod
  ...
  call cint2e_gssp1ssp2(fijkl, shls, atm, natm, bas, nbas, env, 0_8)
  do is = 1, 3
  ...
  vk(n2c+1:,n2c+1:,:) = vk(n2c+1:,n2c+1:,:) * (.5d0/env(PTR_LIGHT_SPEED))**2
  return
end subroutine rkb_giao_vhf_gaunt_O00

subroutine rkb_giao_vhf_gaunt_O01(dm, vj, vk, ndim, atm, natm, bas, nbas, env)
!**************************************************
  use cint_const_mod
  use cint_interface
  use hf_block_mod
  ...
  return
end subroutine rkb_giao_vhf_gaunt_O01
