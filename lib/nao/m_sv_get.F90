...
end subroutine ! sv_get

!
!
!
subroutine test_sv_get(n, svn) bind(c, name='test_sv_get')
  use m_system_vars, only : system_vars_t
  implicit none
  integer(c_int64_t), intent(in) :: n
  real(c_double), intent(in) :: svn(n)
  !! internal
  type(system_vars_t) :: sv

  call sv_get(n, svn, sv)
  
  !write(6,*) 'test_sv_get allocated psi_log: ', allocated(sv%psi_log)

end subroutine ! test_sv_get
