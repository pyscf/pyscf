...
int trianglefunction(const int k, const int glob){

   int cnt = 1;
   while ((cnt*(cnt-1))/2 <= glob){ cnt++; }
   return k - cnt;
...
void hessian_boys(const int Norbs, double * x_symm, double * y_symm, double * z_symm, double * vector_in, double * vector_out){
    ...
    for (p=0; p<Norbs; p++){
        for (q=p+1; q<Norbs; q++){
            ...
            //increment_row == q + p * Norbs - ((p+1)*(p+2))/2
            ...
            for (s=p+1; s<Norbs; s++){
                ...
                const int colindex = increment_row + s - q; // s + p * Norbs - ((p+1)*(p+2))/2
            ...
            vector_out[ increment_row ] = -Norbs*value;
            increment_row += 1;
...
void hessian_edmiston(const int Norbs, double * eri, double * vector_in, double * vector_out){
    ...
    for (p=0; p<Norbs; p++){
        for (q=p+1; q<Norbs; q++){
            ...
            vector_out[ increment_row ] = -value;
            increment_row += 1;
