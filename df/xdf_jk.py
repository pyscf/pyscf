...
def density_fit(mf, auxbasis=None, gs=(10,10,10), with_df=None, jkops=None):
    ...
    class XDFHF(mf_class):
        def __init__(self):
            ...
            self.jkops = jkops
            self._keys = self._keys.union(['with_df', 'jkops'])

        def get_jk(self, mol=None, dm=None, hermi=1):
            if self.with_df:
                return get_jk(self.with_df, mol, dm, hermi, self.opt,
                              True, True, self.jkops)
            else:
...
def get_jk(xdf, mol, dm, hermi=1, vhfopt=None, with_j=True, with_k=True,
           jkops=None):
    log = logger.Logger(xdf.stdout, xdf.verbose)
    t1 = (time.clock(), time.time())
    if xdf.auxmol is None:
        xdf.build()
    ...
    if with_k and hermi and numpy.einsum('ij,ij->', dm, ovlp) > 0.1:
        Lpq = jkops.decompose_dm(dm, ovlp, Lpq)
    ovlp = None

    sublk = min(max(16, int(xdf.max_memory*1e6/16/nao**2)), 8192)
    ...
    for pqkR, LkR, pqkI, LkI, coulG \
            in xdf.pw_loop(mol, auxmol, xdf.gs, xdf.max_memory):
        if with_j:
            vj, jaux = jkops.add_j_(vj, jaux, dm, rho_coeff, pqkR, LkR, coulG)
            vj, jaux = jkops.add_j_(vj, jaux, dm, rho_coeff, pqkI, LkI, coulG)
        if with_k:
            vk = jkops.add_k_(vk, dm, pqkR, LkR, coulG, Lpq, pikbuf)
            vk = jkops.add_k_(vk, dm, pqkI, LkI, coulG, Lpq, pikbuf)
    pqkR = LkR = pqkI = LkI = coulG = pikbuf = None
    t1 = log.timer('pw jk', *t1)

    for j3c, j2c in xdf.sr_loop(mol, auxmol, xdf.max_memory):
        if with_j:
            vj = vj.reshape(nao,nao)
            vj += jkops.sr_vj(jaux, dm, rho_coeff, Lpq, j3c, j2c)
        if with_k:
            vk += jkops.sr_vk(dm, Lpq, j3c, j2c, hermi)
    j3c = j2c = None
...
class _JKContracts:
    ...
    def read_Lpq(self):
        with self.xdf.load('Lpq') as Lpq:
            Lpq = lib.unpack_tril(Lpq.value)
        naux = Lpq.shape[0]
        return Lpq.reshape(naux,-1)

    def decompose_dm(self, dm, ovlp, Lpq):
        nao = dm.shape[0]
        naux = Lpq.shape[0]
    ...
    def sr_vj(self, jaux, dm, rho_coeff, Lpq, j3c, j2c):
        '''Short range'''
        nao = dm.shape[0]
    ...
    def sr_vk(self, dm, Lpq, j3c, j2c, hermi):
        nao = dm.shape[0]
        naux = j2c.shape[0]
    ...
    def add_j_(self, vj, jaux, dm, rho_coeff, pqk, Lk, coulG):
        nG = Lk.shape[1]
    ...
    def add_k_(self, vk, dm, pqk, Lk, coulG, Lpq, buf=None):
        nao = dm.shape[0]
        nG = Lk.shape[1]
        if isinstance(Lpq, tuple):
            cpos, cneg, Lpi_pos, Lpi_neg = Lpq
...
if __name__ == '__main__':
    ...
    #auxbasis = pyscf.df.xdf._default_basis(mol, addons.aug_etb_for_dfbasis(mol, beta=2.2, start_at=0))
    #auxbasis = addons.aug_etb_for_dfbasis(mol, beta=2.2, start_at=0)
    auxbasis = None
    #auxbasis = addons.aug_etb_for_dfbasis(mol, beta=2.2, start_at=0)
    #auxbasis = pyscf.df.xdf._default_basis(mol)
    #mf = scf.density_fit(scf.RHF(mol), auxbasis)
    #mf = with_poisson_(scf.RHF(mol), auxbasis, gs=(50,302))
    mf = xdf.XDF(mol).set(auxbasis=auxbasis, gs=(5,)*3).update_mf(scf.RHF(mol))
