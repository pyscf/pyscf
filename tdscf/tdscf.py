...
class tdscf:
	...
	def numint_vxc(self,ni,P,max_mem = 2000):
		...
		if xctype == 'LDA':
			ao_deriv = 0
			for ao, mask, weight, coords in ni.block_loop(self.mol, self.the_scf.grids, nao, ao_deriv, max_mem):
				...
				vmat += func._dot_ao_ao(self.mol, ao, aow, mask, shls_slice, ao_loc)
			...
			for ao, mask, weight, coords in ni.block_loop(self.mol, self.the_scf.grids, nao, ao_deriv, max_mem):
				...
				vmat += func._dot_ao_ao(self.mol, ao[0], aow, mask, shls_slice, ao_loc)
				den = rho[0] * weight
				excsum[0] += (den * exc).sum()
			...
			for ao, mask, weight, coords in ni.block_loop(self.mol, self.the_scf.grids, nao, ao_deriv, max_mem):
				...
				vmat += func._dot_ao_ao(self.mol, ao[1], wv*ao[1], mask, shls_slice, ao_loc)
				vmat += func._dot_ao_ao(self.mol, ao[2], wv*ao[2], mask, shls_slice, ao_loc)
				vmat += func._dot_ao_ao(self.mol, ao[3], wv*ao[3], mask, shls_slice, ao_loc)
				rho = exc = vxc = vrho = vsigma = wv = aow = None
		self.Exc = excsum[0]
	...
	def InitFockBuild(self):
		...
		#adiis = self.the_scf.diis.DIIS(self.the_scf, self.the_scf.diis_file)
