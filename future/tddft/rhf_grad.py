...
def grad_elec(td_grad, z, atmlst=None, max_memory=2000, verbose=logger.INFO):
    ...
    dvv = numpy.einsum('ai,bi->ab', z, z)
    doo =-numpy.einsum('ai,aj->ij', z, z)
    dmzvo = reduce(numpy.dot, (orbv, z, orbo.T))
    dmzoo = reduce(numpy.dot, (orbo, doo, orbo.T))
    dmzoo+= reduce(numpy.dot, (orbv, dvv, orbv.T))

    vj, vk = mf.get_jk(mol, (dmzoo,dmzvo), hermi=0)
    vhf0 = vj * 2 - vk
    wvo = reduce(numpy.dot, (orbv.T, vhf0[0], orbo)) * 2
    vhf0mo = reduce(numpy.dot, (mo_coeff.T, vhf0[1], mo_coeff))
    wvo -= numpy.einsum('ki,ai->ak', vhf0mo[:nocc,:nocc], z) * 2
    wvo += numpy.einsum('ac,ai->ci', vhf0mo[nocc:,nocc:], z) * 2
    def fvind(x):
    ...
    im0[:nocc,:nocc] = reduce(numpy.dot, (orbo.T, vhf0[0]+vj*2-vk, orbo))
    im0[nocc:,nocc:] = numpy.einsum('ci,ai->ac', vhf0mo[nocc:,:nocc], z)
    im0[:nocc,:nocc]+= numpy.einsum('ak,ai->ki', vhf0mo[nocc:,:nocc], z)
    im0[nocc:,:nocc] = numpy.einsum('ki,ai->ak', vhf0mo[:nocc,:nocc], z) * 2
    ...
    dmz1doo = z1ao + dmzoo
# dmzvo ~ (nabla a p|q i) D_{pq}
# dmzvo.T ~ (a p|q nabla i) D_{pq} = (nabla i q|p a) D_{qp}
    oo0 = reduce(numpy.dot, (orbo, orbo.T))
    vj, vk = td_grad.get_jk(mol, (oo0, dmz1doo+dmz1doo.T, dmzvo, dmzvo.T))
    vhf1 = vj * 2 - vk
    ...
    for k, ia in enumerate(atmlst):
        ...
        e1 += numpy.einsum('xij,ij->x', vhf1[2,:,p0:p1], dmzvo[p0:p1,:]) * 2
        e1 += numpy.einsum('xij,ji->x', vhf1[3,:,p0:p1], dmzvo[:,p0:p1]) * 2
