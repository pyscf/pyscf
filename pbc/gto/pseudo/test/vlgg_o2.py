...
def get_pp(cell, kpt=numpy.zeros(3)):
    ...
    for ia in range(cell.natm):
        ...
        for l, proj in enumerate(pp[5:]):
            ...
            p0 += l * 2 + 1
# real to complex spheric harmonic transformation U can be eliminated here
# because of the contraction over m of Ylm in Eq 4.80 leads to the matrix
# multiplication U U^T = 1
#            pkl_part = _real2cmplx(pkl_part, l)
            ...
            SPG_lmi = numpy.einsum('g,ngm->ngm', SI[ia], pkl)
            SPG_lm_aoG = numpy.einsum('ngm,gp->pnm', SPG_lmi.conj(), aokG)
            vppnl += numpy.einsum('pim,ij,qjm->pq', SPG_lm_aoG.conj(), hl, SPG_lm_aoG)
    vppnl *= (1./ngs**2)
