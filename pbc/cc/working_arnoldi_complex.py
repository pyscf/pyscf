...
def tester():
    ...
    def setA(sizer, plus):
        ...
        fac =1.0
        A = np.random.rand(sizer,sizer) + 1j*np.random.rand(sizer,sizer)
        eigs = np.random.rand(sizer)
        for i in range(sizer):
            eigs[i] += 3.0
        A = np.conj(A.T) * eigs * A
        print "non hermitian norm = ", np.linalg.norm(A - A.T)
        P = np.ones((sizer,1))
        return A, P

    size=50
    A,P=setA(size,0.0+1j*0.0)
    x = np.random.rand(P.shape[0]).reshape(P.shape[0],1)
    w, v = np.linalg.eig(A)
    idx = w.real.argsort()
    w = w[idx]
    print "Now performing my own routine..."
    arnold = arnoldi(multiplyA,x,P,nroots=5)
    eigens = arnold.solve()
    print "Eigenvalues from my routine ", eigens[:5]
    print "First 5 eigenvalues from scipy: ", w[:5]
    return
