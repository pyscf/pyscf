...
from pyscf.pbc.cc import kpoint_intermediates as imdk
...
def kernel(cc, eris, t1=None, t2=None, max_cycle=50, tol=1e-8, tolnormt=1e-6,
    ...
    *local* energy() function."""
    ...
    if t1 is None and t2 is None:
        ...
        nvir = cc.nmo() - nocc
        nkpts = len(cc._kpts)
        t1 = numpy.zeros((nkpts,nocc,nvir), np.complex128)
    elif t2 is None:
...
#
    ...
    for ki in xrange(nkpts):
      for kj in xrange(nkpts):
...
#                #t2new[kj,ki,kb,:,:,:,:] += tmp.transpose(1,0,3,2)
    ...
    for ki in xrange(nkpts):
      for kj in xrange(nkpts):
        for ka in xrange(nkpts):
            kb   = KLMN[ki,ka,kj]  # this is in chemist's notation even though integrals are in physics
                                   #   notation..
            for i in range(nocc):
                for a in range(nvir):
                    for j in range(nocc):
...
class CCSD(pyscf.cc.ccsd.CCSD):
    ...
    def __init__(self, mf, abs_kpts, frozen=[], mo_energy=None, mo_coeff=None, mo_occ=None):
        ...
        if mo_coeff is None:
            ...
            so_coeffT = numpy.zeros((nkpts,nso,nso),dtype=complex)
            mo_coeffT = numpy.zeros((nkpts,nmo,nmo),dtype=complex)
            mo_coeff  = numpy.zeros((nkpts,nso,nso),dtype=complex)
            for k in xrange(nkpts):
    ...
    def nocc(self):
        ...
        #      .. as of right now it works, but just not sure how
        #      the frozen list will work with it
    ...
    def init_amps(self, eris):
        ...
        KLMN = tools.get_KLMN(self._scf.cell,self._kpts)
        for ki in xrange(nkpts):
          for kj in xrange(nkpts):
            for ka in xrange(nkpts):
                kb   = KLMN[ki,ka,kj]  # this is in chemist's notation even though integrals are in physics
...
class _ERIS:
    ...
    def __init__(self, cc, mo_coeff=None, method='incore'):
        ...
        if mo_coeff is None:
            ...
            self.mo_coeff = numpy.zeros((nkpts,nmo,nmo),dtype=complex)
            for k in range(nkpts):
        ...
        so_coeff = numpy.zeros((nkpts,nmo/2,nmo),dtype=complex)
        ...
        if (method == 'incore' and cc._scf._eri is None and
            ...
            eri = numpy.zeros((nkpts,nkpts,nkpts,nmo,nmo,nmo,nmo),dtype=complex)
            ...
            eri[:,:,:,::2,1::2] = eri[:,:,:,1::2,::2] = eri[:,:,:,:,:,::2,1::2] = eri[:,:,:,:,:,1::2,::2] = 0. + 1j*0.0
...
#
            ...
            print "DIFFERENCE = %.15g" % maxdiff

            #print "ERI ="
            #print eri
            eri1 = eri - eri.transpose(2,1,0,5,4,3,6)     # antisymmetrizing    (pq|rs) - (ps|rq), where the latter integral
                                                          #                     is equal to just (rq|ps); done since we
                                                          #                     aren't tracking the kpoint of orbital 's'
            eri1 = eri1.transpose(0,2,1,3,5,4,6)          # chemist -> physics notation

            self.dtype = eri1.dtype
            self.oooo = eri1[:,:,:,:nocc,:nocc,:nocc,:nocc].copy() / (1.*nkpts)
            self.ooov = eri1[:,:,:,:nocc,:nocc,:nocc,nocc:].copy() / (1.*nkpts)
            self.ovoo = eri1[:,:,:,:nocc,nocc:,:nocc,:nocc].copy() / (1.*nkpts)
            self.oovv = eri1[:,:,:,:nocc,:nocc,nocc:,nocc:].copy() / (1.*nkpts)
            self.ovov = eri1[:,:,:,:nocc,nocc:,:nocc,nocc:].copy() / (1.*nkpts)
            self.ovvv = eri1[:,:,:,:nocc,nocc:,nocc:,nocc:].copy() / (1.*nkpts)
            self.vvvv = eri1[:,:,:,nocc:,nocc:,nocc:,nocc:].copy() / (1.*nkpts)
