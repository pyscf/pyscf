...
class KnowValues(unittest.TestCase):
    ...
    def test_get_eri_gamma(self):
        ...
        eri0000 = odf.get_eri()
        self.assertTrue(eri0000.dtype == numpy.double)
        self.assertAlmostEqual(eri0000.real.sum(), 140.52064546906715, 9)
        self.assertAlmostEqual(finger(eri0000), -1.2233565424167161, 9)

        eri1111 = kmdf.get_eri((kpts[0],kpts[0],kpts[0],kpts[0]))
        self.assertTrue(eri1111.dtype == numpy.double)
        self.assertAlmostEqual(eri1111.real.sum(), 140.52064546906803, 9)
        self.assertAlmostEqual(eri1111.imag.sum(), 0, 9)
        self.assertAlmostEqual(finger(eri1111), -1.2233565424167161, 9)
        ...
        self.assertAlmostEqual(eri4444.real.sum(), 259.45213423314976, 9)
        self.assertAlmostEqual(abs(eri4444.imag).sum(), 0.00044187067330929289, 9)
        self.assertAlmostEqual(finger(eri4444), 1.9680747941278547-3.6084192901803842e-07j, 8)
    ...
    def test_get_eri_1111(self):
        ...
        self.assertAlmostEqual(eri1111.real.sum(), 258.80548867896516, 9)
        self.assertAlmostEqual(abs(eri1111.imag).sum(), 16.275865549586005, 9)
        self.assertAlmostEqual(finger(eri1111), 2.2314693640740373+0.10954489557778171j, 9)
    ...
    def test_get_eri_0011(self):
        ...
        self.assertAlmostEqual(eri0011.real.sum(), 259.11748670189525, 9)
        self.assertAlmostEqual(abs(eri0011.imag).sum(), 8.4042428104966493, 9)
        self.assertAlmostEqual(finger(eri0011), 2.1350471926872832+0.12350330021015966j, 9)

    def test_get_eri_0110(self):
        eri0110 = kmdf.get_eri((kpts[0],kpts[1],kpts[1],kpts[0]))
        self.assertTrue(eri0110.dtype == numpy.complex128)
        self.assertAlmostEqual(eri0110.real.sum(), 411.84617685870955, 9)
        self.assertAlmostEqual(abs(eri0110.imag).sum(), 136.58716537520172, 9)
        self.assertAlmostEqual(finger(eri0110), 1.3742577708662935+0.12356180830023202j, 9)
    ...
    def test_get_eri_0123(self):
        ...
        self.assertAlmostEqual(eri0123.real.sum(), 410.38590725156291, 9)
        self.assertAlmostEqual(abs(eri0123.imag.sum()), 0.18511037268491518, 9)
        self.assertAlmostEqual(finger(eri0123), 1.7615505570266121+0.30815052383825764j, 9)
