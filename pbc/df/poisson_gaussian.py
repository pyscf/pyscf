...
if __name__ == '__main__':
    ...
    auxbasis = {'He': df.genbas(3.3, 1.8, (1000000.,0.1), 0)
    ...
    auxcell.nimgs = [2,2,2]
    #auxcell.nimgs = [4,4,4]
    mf.get_j = lambda cell, dm, *args: get_j(cell, dm, auxcell, None)
    mf.get_hcore = lambda cell, *args: get_nuc(cell, auxcell, None) + pscf.hf.get_t(cell)
    e1 = mf.scf()
    print e1 # ~ -4.32022187118
#    print mf.mo_energy

    mf1 = pyscf.dft.RKS(cell)
    mf1.kernel()
    dm = mf1.make_rdm1()
    vj1 = get_j(cell, dm, auxcell, None)
    v1 = get_nuc(cell, auxcell, None)
    print vj1 + v1
    vj0 = mf.get_j(cell, dm)
    v0 = pscf.hf.get_nuc(cell)
    print v0 + vj0
