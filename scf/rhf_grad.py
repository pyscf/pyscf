...
def get_hcore(mol):
    h =(mol.intor('int1e_ipkin_sph', comp=3)
...
def get_ovlp(mol):
    return -mol.intor('int1e_ipovlp_sph', comp=3)

def get_jk(mol, dm):
    '''J = ((-nabla i) j| kl) D_lk
    K = ((-nabla i) j| kl) D_jk
    '''
    vj, vk = _vhf.direct_mapdm('int2e_ip1_sph',  # (nabla i,j|k,l)
...
class Gradients(lib.StreamObject):
    ...
    def get_j(self, mol=None, dm=None, hermi=0):
        if mol is None: mol = self.mol
        if dm is None: dm = self._scf.make_rdm1()
        return -_vhf.direct_mapdm('int2e_ip1_sph', 's2kl', 'lk->s1ij', dm, 3,
                                  mol._atm, mol._bas, mol._env)

    def get_k(self, mol=None, dm=None, hermi=0):
        if mol is None: mol = self.mol
        if dm is None: dm = self._scf.make_rdm1()
        return -_vhf.direct_mapdm('int2e_ip1_sph', 's2kl', 'jk->s1il', dm, 3,
                                  mol._atm, mol._bas, mol._env)
    ...
    def _grad_rinv(self, mol, ia):
        ...
        return -mol.atom_charge(ia) * mol.intor('int1e_iprinv_sph', comp=3)
