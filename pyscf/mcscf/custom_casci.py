...
def h1e_for_cas(casci, mo_coeff=None, ncas=None, ncore=None):
    ...
    if mo_core.size == 0:
        ...
        energy_core += numpy.einsum("ij,ji", core_dm, corevhf).real * 0.5
...
def analyze(
    ...
    **kwargs
    ...
    if isinstance(ci, (list, tuple, RANGE_TYPE)) and not isinstance(
        casscf.fcisolver, addons.StateAverageFCISolver
    ):
        log.warn(
    ...
    if getattr(casscf.fcisolver, "make_rdm1s", None):
        ...
        dm1a = numpy.dot(mocore, mocore.conj().T) * 2 + reduce(
            numpy.dot, (mocas, casdm1, mocas.conj().T)
        )
        dm1b = None
        dm1 = dm1a
    ...
    if log.verbose >= logger.INFO:
        ...
        if ci is not None and (
            getattr(casscf.fcisolver, "large_ci", None)
            or getattr(casscf.fcisolver, "states_large_ci", None)
        ):
            ...
            if isinstance(ci, (list, tuple, RANGE_TYPE)):
                if hasattr(casscf.fcisolver, "states_large_ci"):
                    ...
                    res = casscf.fcisolver.states_large_ci(
                        ci, casscf.ncas, casscf.nelecas, large_ci_tol, return_strs=False
                    )
                else:
                    res = [
                        casscf.fcisolver.large_ci(
                            civec,
                            casscf.ncas,
                            casscf.nelecas,
                            large_ci_tol,
...
def get_fock(mc, mo_coeff=None, ci=None, eris=None, casdm1=None, verbose=None):
    ...
    if getattr(eris, "ppaa", None) is not None:
        ...
        fock = mc.get_hcore() + reduce(
            numpy.dot, (mo_inv.conj().T, eris.vhf_c + vj - vk * 0.5, mo_inv)
        )
    else:
...
def cas_natorb(
    ...
    if occ2_idx.size > 0 or occ0_idx.size > 0:
        ...
        if occ2_idx.size > 0:
            log.warn(
                "Active orbitals %s (occs = %s) are canonicalized with core orbitals",
                occ2_idx,
                cas_occ[occ2_idx],
            )
        ...
        if occ0_idx.size > 0:
            log.warn(
                "Active orbitals %s (occs = %s) are canonicalized with external orbitals",
                occ0_idx,
                cas_occ[occ0_idx],
            )
    ...
    if getattr(mc.fcisolver, "transform_ci_for_orbital_rotation", None):
        ...
        elif isinstance(ci, (list, tuple)) and all(
            isinstance(x[0], numpy.ndarray) for x in ci
        ):
            fcivec = [
                mc.fcisolver.transform_ci_for_orbital_rotation(x, ncas, nelecas, ucas)
                for x in ci
            ]
    ...
    if fcivec is None:
        ...
        if getattr(eris, "ppaa", None) is not None:
            ...
            h1eff += reduce(
                numpy.dot, (ucas.conj().T, eris.vhf_c[ncore:nocc, ncore:nocc], ucas)
            )
            ...
            ecore += numpy.einsum("ij,ji", dm_core, corevhf) * 0.5
        ...
        e, fcivec = mc.fcisolver.kernel(
            h1eff, aaaa, ncas, nelecas, ecore=ecore, max_memory=max_memory, verbose=log
        )
    ...
    if log.verbose >= logger.INFO:
        ...
        if with_meta_lowdin:
            ...
            mo_cas = reduce(
                numpy.dot, (orth_coeff.conj().T, ovlp_ao, mo_coeff1[:, ncore:nocc])
            )
        else:
            log.info("Natural orbital (expansion on AOs) in CAS space")
            label = mc.mol.ao_labels()
        ...
        if mc._scf.mo_coeff is not None:
            ...
            for i, j in idx:
                log.info(
                    "<CAS-nat-orb|mo-hf>  %d  %d  %12.8f", ncore + i + 1, j + 1, s[i, j]
                )
...
def canonicalize(
    ...
    if casdm1 is None:
        if isinstance(ci, (list, tuple, RANGE_TYPE)) and not isinstance(
            ...
            if stav_dm1:
                log.warn(
                    "Mulitple states found in CASCI solver. "
                ...
                for root in range(1, len(ci)):
                    casdm1 += mc.fcisolver.make_rdm1(ci[root], mc.ncas, mc.nelecas)
                casdm1 /= len(ci)
            else:
                log.warn(
                    "Mulitple states found in CASCI solver. "
    ...
    if cas_natorb:
        mo_coeff1, ci, mc.mo_occ = mc.cas_natorb(
            mo_coeff, ci, eris, sort, casdm1, verbose, with_meta_lowdin
        )
    else:
...
def kernel(
    casci, mo_coeff=None, ci0=None, verbose=logger.NOTE, envs=None, initial_state=None
):
    """CASCI solver
    ...
    e_tot, fcivec, circ = casci.fcisolver.kernel(
        h1eff,
        eri_cas,
        ncas,
        nelecas,
    ...
    return e_tot, e_cas, fcivec, circ
...
class CASCI(lib.StreamObject):
    ...
    sorting_mo_energy = getattr(
        __config__, "mcscf_casci_CASCI_sorting_mo_energy", False
    )
    ...
    def __init__(self, mf_or_mol, ncas, nelecas, ncore=None):
        ...
        self.ncore = ncore
        singlet = (
            getattr(__config__, "mcscf_casci_CASCI_fcisolver_direct_spin0", False)
        ...
        self.fcisolver.lindep = getattr(
            __config__, "mcscf_casci_CASCI_fcisolver_lindep", 1e-10
        )
    ...
    def dump_flags(self, verbose=None):
        ...
        if self.mo_coeff is None:
            log.error(
                "Orbitals for CASCI are not specified. The relevant SCF "
                "object may not be initialized."
            )

        if getattr(self._scf, "with_solvent", None) and not getattr(
            self, "with_solvent", None
        ):
            log.warn(
                """Solvent model %s was found at SCF level but not applied to the CASCI object.
...
""",
    ...
    def ao2mo(self, mo_coeff=None):
        ...
        if self._scf._eri is not None:
            eri = ao2mo.full(self._scf._eri, mo_coeff, max_memory=self.max_memory)
        else:
            eri = ao2mo.full(
                self.mol, mo_coeff, verbose=self.verbose, max_memory=self.max_memory
            )
        return eri
    ...
    def kernel(self, mo_coeff=None, ci0=None, verbose=None):
        ...
        self.e_tot, self.e_cas, self.ci = kernel(self, mo_coeff, ci0=ci0, verbose=log)
        ...
        if log.verbose >= logger.NOTE and getattr(self.fcisolver, "spin_square", None):
            if isinstance(self.e_cas, (float, numpy.number)):
                ...
                for i, e in enumerate(self.e_cas):
                    ...
                    log.note(
                        "CASCI state %d  E = %.15g  E(CI) = %.15g  S^2 = %.7f",
                        i,
                        self.e_tot[i],
                        e,
                        ss[0],
            ...
            if isinstance(self.e_cas, (float, numpy.number)):
                ...
                for i, e in enumerate(self.e_cas):
                    log.note(
                        "CASCI state %d  E = %.15g  E(CI) = %.15g", i, self.e_tot[i], e
                    )
        return self
    ...
    def cas_natorb(
        ...
        return cas_natorb(
            self, mo_coeff, ci, eris, sort, casdm1, verbose, with_meta_lowdin
        )
    ...
    def cas_natorb_(
        ...
        self.mo_coeff, self.ci, self.mo_occ = cas_natorb(
            self, mo_coeff, ci, eris, sort, casdm1, verbose
        )
    ...
    def canonicalize_(
        ...
        if (
            cas_natorb
        ):  # When active space is changed, the ci solution needs to be updated
            self.ci = ci
    ...
    def make_rdm1s(
        self, mo_coeff=None, ci=None, ncas=None, nelecas=None, ncore=None, **kwargs
    ):
