...
def project_init_guess_alt(casscf, mo_init, priority='active'):
    ...
    Kwargs:
        priority : 'active', 'core', nested idx arrays, or mask arrays
            ...
            or equivalently by numpy.eye (nmo, dtype = bool). Within 
            each row of the mask array, the orbitals will be orth'd by
            SVD. Missing orbitals will be appended in a single row at
            the bottom of the mask array. A 3-dimensional nested
            list/mask array can be passed if and only if UHF-CASSCF is
            used.
    ...
    def _check_range (priority, ncore):
        ...
        if isinstance (priority, str):
            ridx = numpy.zeros ((3, nmo), dtype=numpy.bool)
            if priority == 'active':
            ...
            assert (priority.shape[0] == nmo), errstr
        ...
        if numpy.any (ridx_counts == 0):
            ridx = numpy.vstack ([ridx, ridx_counts==0])
        return ridx
