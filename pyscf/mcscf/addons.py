...
def project_init_guess_alt(casscf, mo_init, priority='active'):
    '''Project the given initial guess to the current CASSCF problem 
    giving priority to the active orbitals and using SVD on the
    orthogonal complement of the active orbitals to find the core
    orbitals. The Hartree-Fock orbitals are only used as an orthonormal
    basis.
    ...
    Args:
        ...
        mo_init : ndarray or list of ndarray
            ...
            molecule.  When the casscf is UHF-CASSCF, the mo_init needs to be
            a list of two ndarrays, for alpha and beta orbitals

    Kwarge:
        priority : 'active', 'core', or list of list of integers
            Specifies the order in which groups of orbitals are proj'd.
            Orbitals orthogonalized first are deformed less than those
            orthogonalized later because the latter have to "move out of
            the way" of the former. 'core' means core, then active, then
            virtual; 'active' means active, then core, then virtual, and
            the Gram-Schmidt process is generated by [[0],[1],[2],...].
            Within each element (i.e., list of integers) the orbitals
            will be orthogonalized by SVD. Missing orbitals will be
            appended in a single element at the end of the list.
            A 3-dimensional nested list of integers can be passed if and
            only if UHF-CASSCF is used.
    ...
    if numpy.asarray (mo_init).shape[0] != numpy.asarray (mf_mo).shape[0]:
    ...
    def _spincase (mo_basis, mo_target, range_idx):
        # Avoid scrambling the order of the target orbitals
        mo = numpy.zeros_like (mo_basis)
        for idx in range_idx:
            mo[:,idx], mo_basis = _rangecase (mo_basis, mo_target[:,idx])
        assert (mo_basis.shape[1] == 0), 'incomplete basis in mo_init?'
        ...
        if casscf.verbose >= logger.DEBUG:
            ...
            #mo_t_n = mo_target / numpy.sqrt (tnorm)[None,:]
            #s1_norm = reduce(numpy.dot, (mo.T, s0, mo_t_n))
            s1_norm = s1 / numpy.sqrt (tnorm) [None,:]
            idx = numpy.argmax (numpy.abs (s1), axis=0)
            for i, j in enumerate (idx):
                logger.debug(casscf, 'Init guess <mo-orth|mo-init>  %d  %d  %12.8f (%12.8f after norm)',
                             i+1, j+1, s1[i,j], s1_norm[i,j])
        return mo

    # Check validity of range lists (from "priority" keyword)
    # Add missing orbitals to the end, if any
    def _check_range (ridx, ncore):
        # Interpret priority keyword
        nocc = ncore + ncas
        if isinstance (ridx, str):
            if ridx == 'active':
                ridx = [list (range (ncore,nocc)),
                        list (range (0,ncore)),
                        list (range (nocc,nmo))]
            elif ridx == 'core':
                ridx = [list (range (0,ncore)),
                        list (range (ncore,nocc)),
                        list (range (nocc,nmo))]
            else:
                raise RuntimeError ("Invalid priority keyword ('active', 'core', or nested list required)")
        ridx_flat = numpy.concatenate (ridx)
        if numpy.any (numpy.unique (ridx_flat, return_counts=True)[-1] > 1):
            raise RuntimeError ("Invalid priority keyword (repeated elements)")
        if numpy.amin (ridx_flat) < 0 or numpy.amax (ridx_flat) >= nmo:
            raise RuntimeError ("Invalid priority keyword (elements outside of range 0 <= i < nmo)")
        all_idx = numpy.arange (0, nmo, dtype=ridx_flat.dtype)
        missing = numpy.setdiff1d (all_idx, ridx_flat).tolist ()
        if len (missing) > 0:
            ridx.append (missing)
        return ridx
    ...
    if isinstance(ncore, (int, numpy.integer)):
        ...
        if not isinstance (priority, str):
            assert (isinstance (priority[0][0], (int, numpy.integer))), errstr
        mo = _spincase (mf_mo, mo_init, _check_range (priority, ncore))
    else: # UHF-based CASSCF
        if (isinstance (priority, str) or isinstance (priority[0][0], (int, numpy.integer))):
            priority = [priority, priority]
        else:
            chk = isinstance (priority, str)
            chk = chk or isinstance (priority[0][0][0], (int, numpy.integer))
            chk = chk and len (priority) == 2
            assert (chk), 'Invalid priority keyword (unintelligible shape)'
        mo = (_spincase (mf_mo[0], mo_init[0], _check_range (priority[0], ncore[0])),
              _spincase (mf_mo[1], mo_init[1], _check_range (priority[1], ncore[1])))
