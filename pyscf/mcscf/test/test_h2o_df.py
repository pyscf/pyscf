...
m = scf.RHF(mol).density_fit (auxbasis = 'weigend')
m.conv_tol = 1e-10
m.scf()
...
msym = scf.RHF(molsym).density_fit (auxbasis = 'weigend')
msym.conv_tol = 1e-10
msym.scf()
...
mc_ss_ref = mcscf.CASSCF (m, 4, 4).run ()
mcn_ss_ref = mcscf.CASSCF (m, 4, 4).newton ().run (mc_ss_ref.mo_coeff)
print (mc_ss_ref.e_tot, mcn_ss_ref.e_tot, mc_ss_ref.e_tot - mcn_ss_ref.e_tot)
...
class KnownValues(unittest.TestCase):
    def test_nosymm_sa4_newton (self):
        ...
        for e1, e0 in zip (mc.e_states, mc_ref.e_states):
            print (e1, e0, e1-e0)
        self.assertAlmostEqual (mc.e_tot, mc_ref.e_tot, 9)
        for e1, e0 in zip (mc.e_states, mc_ref.e_states):
