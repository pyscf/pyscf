...
class KnownValues(unittest.TestCase):
    ...
    def test_ab_hf(self):
        ...
        numpy.random.seed(2)
        xa, ya = numpy.random.random((2,nvir_a,nocc_a))
        xb, yb = numpy.random.random((2,nvir_b,nocc_b))
        x = numpy.hstack((xa.ravel(), xb.ravel()))
        y = numpy.hstack((ya.ravel(), yb.ravel()))
        xy = numpy.hstack((x, y))
        ax_a = numpy.einsum('iajb,bj->ai', a[0], xa)
        ax_a+= numpy.einsum('iajb,bj->ai', a[1], xb)
        ax_b = numpy.einsum('jbia,bj->ai', a[1], xa)
        ax_b+= numpy.einsum('iajb,bj->ai', a[2], xb)
        ax = numpy.hstack((ax_a.ravel(), ax_b.ravel()))
        self.assertAlmostEqual(abs(ax - ftda([x])).max(), 0, 9)

        ay_a = numpy.einsum('iajb,bj->ai', a[0], ya)
        ay_a+= numpy.einsum('iajb,bj->ai', a[1], yb)
        ay_b = numpy.einsum('jbia,bj->ai', a[1], ya)
        ay_b+= numpy.einsum('iajb,bj->ai', a[2], yb)
        ay = numpy.hstack((ay_a.ravel(), ay_b.ravel()))

        bx_a = numpy.einsum('iajb,bj->ai', b[0], xa)
        bx_a+= numpy.einsum('iajb,bj->ai', b[1], xb)
        bx_b = numpy.einsum('jbia,bj->ai', b[1], xa)
        bx_b+= numpy.einsum('iajb,bj->ai', b[2], xb)
        bx = numpy.hstack((bx_a.ravel(), bx_b.ravel()))

        by_a = numpy.einsum('iajb,bj->ai', b[0], ya)
        by_a+= numpy.einsum('iajb,bj->ai', b[1], yb)
        by_b = numpy.einsum('jbia,bj->ai', b[1], ya)
        by_b+= numpy.einsum('iajb,bj->ai', b[2], yb)
    ...
    def test_analyze(self):
        ...
        self.assertAlmostEqual(lib.finger(dip), 0.09760324780325072, 9)
        f = td.oscillator_strength(gauge='length')
        self.assertAlmostEqual(lib.finger(f), -0.00034747191841310503, 9)
        f = td.oscillator_strength(gauge='velocity', order=2)
        self.assertAlmostEqual(lib.finger(f), -0.0043239860814561762, 9)
        td.analyze()
