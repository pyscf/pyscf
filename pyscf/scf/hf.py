...
def kernel(mf, conv_tol=1e-10, conv_tol_grad=None,
    ...
    Kwargs:
        ...
        sapgrasp : str (small | large)
            SAP initial guess coefficient set size.
...
employing the updated GWH rule from doi:10.1021/ja00480a005.''')
    ...
    def init_guess_by_vsap(self, mol=None, sapgrasp='small'):
        if mol is None: mol = self.mol
...
using procedure laid out by S. Lehtola at doi:10.1021/acs.jctc.8b01089.''')
        return init_guess_by_vsap(mol, sapgrasp=sapgrasp)
    ...
    def get_init_guess(self, mol=None, key='minao', **kwargs):
        ...
        if key == '1e' or key == 'hcore':
            ...
            if 'sapgrasp' not in kwargs:
                sapgrasp = 'small'
            else:
                sapgrasp = kwargs['sapgrasp']
            dm = self.init_guess_by_vsap(mol, sapgrasp=sapgrasp)
        elif key[:3] == 'chk':
            try:
