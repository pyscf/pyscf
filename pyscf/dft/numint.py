...
def nr_uks(ni, mol, grids, xc_code, dms, relativity=0, hermi=0,
    ...
    if xctype == 'LDA':
        ...
        for i, rho, ao, mask, weight, vxc in block_loop(ao_deriv):
            ...
            vmat[0,i] += _dot_ao_ao(mol, ao, aow, mask, shls_slice, ao_loc)
            #:aow = numpy.einsum('pi,p->pi', ao, .5*weight*vrho[:,1], out=aow)
            aow = _scale_ao(ao, .5*weight*vrho[:,1], out=aow)
            vmat[1,i] += _dot_ao_ao(mol, ao, aow, mask, shls_slice, ao_loc)
...
def nr_rks_fxc_st(ni, mol, grids, xc_code, dm0, dms_alpha, relativity=0, singlet=True,
    ...
    make_rho1, nset, nao = ni._gen_rho_evaluator(mol, dms_alpha, hermi=0)
    if rho0 is None and (xctype != 'LDA' or fxc is None):
        make_rho0a = ni._gen_rho_evaluator(mol, dm0*.5, hermi=1)[0]
    else:
        make_rho0a = None
    ...
    if xctype == 'LDA':
        ...
        for ao, mask, weight, coords \
            ...
            if fxc is None:
                ...
                _fxc = ni.eval_xc(xc_code, (rhoa,rhoa), spin=1, deriv=2)[2]
                u_u, u_d, d_d = _fxc[0].T
            else:
                if fxc[0].ndim == 1:
                    raise RuntimeError('cached (rho, vxc, fxc) need to be '
                                       'generated by cache_xc_kernel with flag spin=1')
                u_u, u_d, d_d = fxc[0][p0:p1].T
            if singlet:
                frho = u_u + u_d
            else:
                frho = u_u - u_d
        ...
        for ao, mask, weight, coords \
            ...
            if vxc is None or fxc is None:
                ...
                _fxc = [None if x is None else x[p0:p1] for x in fxc]
            vsigma = _vxc[1].T
            u_u, u_d, d_d = _fxc[0].T  # v2rho2
            u_uu, u_ud, u_dd, d_uu, d_ud, d_dd = _fxc[1].T  # v2rhosigma
            uu_uu, uu_ud, uu_dd, ud_ud, ud_dd, dd_dd = _fxc[2].T  # v2sigma2
            ...
            for i in range(nset):
                ...
                # rho1[1:] = \nabla(|b><j|) z_{bj}
                rho1 = make_rho1(i, ao, mask, xctype)
                wv = _rks_gga_wv1(rhoa, rho1, _vxc, _fxc, weight)
                #:aow = numpy.einsum('npi,np->pi', ao, wv, out=aow)
                aow = _scale_ao(ao, wv, out=aow)
                vmat[i] += _dot_ao_ao(mol, aow, ao[0], mask, shls_slice, ao_loc)
                rho1 = None
        ...
        for ao, mask, weight, coords \
            ...
            if singlet:
                ...
                ftt = _tt[0] - _tt[1]  # u_u - u_d
                frt = _rt[0] - _rt[1]  # u_u - u_d
                fgt = _gt[0] - _gt[4]  # uu_u - dd_u
            _vxc = (None, vsigma, None, None)
            _fxc = (frr, frg, fgg, None, ftt, None, frt, None, None, fgt)

            for i in range(nset):
                # rho1[0 ] = |b><j| z_{bj}
                # rho1[1:] = \nabla(|b><j|) z_{bj}
                rho1 = make_rho1(i, ao, mask, xctype)
                wv = _rks_mgga_wv1(rhoa, rho1, _vxc, _fxc, weight)
                #:aow = numpy.einsum('npi,np->pi', ao, wv, out=aow)
                aow = _scale_ao(ao[:4], wv[:4], out=aow)
                vmat[i] += _dot_ao_ao(mol, aow, ao[0], mask, shls_slice, ao_loc)
                vmat[i] += _dot_ao_ao(mol, ao[1], _scale_ao(ao[1], wv[5], aow), mask, shls_slice, ao_loc)
...
def nr_uks_fxc(ni, mol, grids, xc_code, dm0, dms, relativity=0, hermi=0,
    ...
    if xctype == 'LDA':
        ...
        aow = None
        for i, _rho0, rho1, ao, mask, weight, _vxc, _fxc in block_loop(ao_deriv):
            wva, wvb = _uks_gga_wv1(_rho0, rho1, _vxc, _fxc, weight)
            #:aow = numpy.einsum('npi,np->pi', ao, wva, out=aow)
            aow = _scale_ao(ao, wva, out=aow)
...
def _uks_gga_wv2(rho0, rho1, fxc, kxc, weight):
    ...
    wva[0] += u_d_ud * rho1b[0] * a0b1 * 2
    wva[0] += u_u_ud * rho1a[0] * b0a1 * 2
    ...
    wva[0] += u_uu_dd * a0a1_b0b1 * 8
    wva[0] += u_ud_ud * (a0b1 + b0a1)**2
    ...
    wvb[0] += u_d_ud * rho1a[0] * b0a1 * 2
    wvb[0] += d_d_ud * rho1b[0] * a0b1 * 2
    ...
    wvb[0] += d_ud_ud * (a0b1 + b0a1)**2
    wvb[0] += d_uu_dd * b0b1_a0a1 * 8
...
def _uks_mgga_wv1(rho0, rho1, vxc, fxc, weight):
    ...
    wva[1:4]+= uu_uu * a0a1 * rho0a[1:4] * 4
    wva[1:4]+= uu_ud * b0a1 * rho0a[1:4] * 2
    wva[1:4]+= uu_ud * a0a1 * rho0b[1:4] * 2
    wva[1:4]+= ud_ud * b0a1 * rho0b[1:4]
    ...
    wva[1:4]+= uu_ud * a0b1 * rho0a[1:4] * 2
    wva[1:4]+= ud_ud * a0b1 * rho0b[1:4]
    wva[1:4]+= uu_dd * b0b1 * rho0a[1:4] * 4
    wva[1:4]+= ud_dd * b0b1 * rho0b[1:4] * 2
    ...
    wvb[1:4]+= ud_dd * b0a1 * rho0b[1:4] * 2
    wvb[1:4]+= ud_ud * b0a1 * rho0a[1:4]
    wvb[1:4]+= uu_dd * a0a1 * rho0b[1:4] * 4
    wvb[1:4]+= uu_ud * a0a1 * rho0a[1:4] * 2
    ...
    wvb[1:4]+= dd_dd * b0b1 * rho0b[1:4] * 4
    wvb[1:4]+= ud_dd * b0b1 * rho0a[1:4] * 2
    wvb[1:4]+= ud_dd * a0b1 * rho0b[1:4] * 2
    wvb[1:4]+= ud_ud * a0b1 * rho0a[1:4]
...
def _uks_mgga_wv2(rho0, rho1, fxc, kxc, weight):
    ...
    wva[0] += u_u_d * rho1a[0] * rho1b[0] * 2
    wva[0] += u_d_d * rho1b[0] * rho1b[0]
    wva[0] += frrt[0] * rho1a[0] * rho1a[5] * 2
    wva[0] += frrt[1] * rho1a[0] * rho1b[5] * 2
    wva[0] += frrt[2] * rho1b[0] * rho1a[5] * 2
    wva[0] += frrt[3] * rho1b[0] * rho1b[5] * 2
    wva[0] += u_uu * a1a1 * 2
    wva[0] += u_ud * a1b1 * 2
    wva[0] += u_dd * b1b1 * 2
    wva[0] += u_u_uu * rho1a[0] * a0a1 * 4
    ...
    wva[0] += u_uu_uu * a0a1_a0a1 * 4
    wva[0] += u_uu_ud * a0a1_b0a1 * 4
    wva[0] += u_uu_ud * a0b1_a0a1 * 4
    wva[0] += u_uu_dd * a0a1_b0b1 * 8
    wva[0] += u_ud_ud * (a0b1 + b0a1)**2
    wva[0] += u_ud_dd * b0a1_b0b1 * 4
    wva[0] += u_ud_dd * a0b1_b0b1 * 4
    wva[0] += u_dd_dd * b0b1_b0b1 * 4
    wva[0] += frtt[0] * rho1a[5] * rho1a[5]           # u_u_u
    wva[0] += frtt[1] * rho1a[5] * rho1b[5] * 2       # u_u_d
    wva[0] += frtt[2] * rho1b[5] * rho1b[5]           # u_d_d
    wva[0] += frgt[0] * rho1a[5] * a0a1 * 4           # u_uu_u
    wva[0] += frgt[1] * rho1b[5] * a0a1 * 4           # u_uu_d
    wva[0] += frgt[2] * rho1a[5] * (a0b1 + b0a1) * 2  # u_ud_u
    wva[0] += frgt[3] * rho1b[5] * (a0b1 + b0a1) * 2  # u_ud_d
    wva[0] += frgt[4] * rho1a[5] * b0b1 * 4           # u_dd_u
    wva[0] += frgt[5] * rho1b[5] * b0b1 * 4           # u_dd_d
    ...
    wva[1:4] += u_u_ud * rho1a[0] * rho1a[0] * rho0b[1:4]
    wva[1:4] += u_d_ud * rho1a[0] * rho1b[0] * rho0b[1:4] * 2
    wva[1:4] += d_d_ud * rho1b[0] * rho1b[0] * rho0b[1:4]
    wva[1:4] += frgt[0] * rho1a[5] * rho1a[0] * rho0a[1:4] * 2  # u_uu_u
    wva[1:4] += frgt[1] * rho1b[5] * rho1a[0] * rho0a[1:4] * 2  # u_uu_d
    wva[1:4] += frgt[2] * rho1a[5] * rho1a[0] * rho0b[1:4]      # u_ud_u
    wva[1:4] += frgt[3] * rho1b[5] * rho1a[0] * rho0b[1:4]      # u_ud_d
    wva[1:4] += frgt[6] * rho1a[5] * rho1b[0] * rho0a[1:4] * 2  # d_uu_u
    wva[1:4] += frgt[7] * rho1b[5] * rho1b[0] * rho0a[1:4] * 2  # d_uu_d
    wva[1:4] += frgt[8] * rho1a[5] * rho1b[0] * rho0b[1:4]      # d_ud_u
    wva[1:4] += frgt[9] * rho1b[5] * rho1b[0] * rho0b[1:4]      # d_ud_d
    ...
    wva[1:4] += fgtt[0] * rho1a[5] * rho1a[5] * rho0a[1:4] * 2  # uu_u_u
    wva[1:4] += fgtt[1] * rho1a[5] * rho1b[5] * rho0a[1:4] * 4  # uu_u_d
    wva[1:4] += fgtt[2] * rho1b[5] * rho1b[5] * rho0a[1:4] * 2  # uu_d_d
    wva[1:4] += fgtt[3] * rho1a[5] * rho1a[5] * rho0b[1:4]      # ud_u_u
    wva[1:4] += fgtt[4] * rho1a[5] * rho1b[5] * rho0b[1:4] * 2  # ud_u_d
    wva[1:4] += fgtt[5] * rho1b[5] * rho1b[5] * rho0b[1:4]      # ud_d_d
    wva[1:4] += frgt[0] * rho1a[0] * rho1a[5] * rho0a[1:4] * 2  # u_uu_u
    wva[1:4] += frgt[1] * rho1a[0] * rho1b[5] * rho0a[1:4] * 2  # u_uu_d
    wva[1:4] += frgt[2] * rho1a[0] * rho1a[5] * rho0b[1:4]      # u_ud_u
    wva[1:4] += frgt[3] * rho1a[0] * rho1b[5] * rho0b[1:4]      # u_ud_d
    wva[1:4] += frgt[6] * rho1b[0] * rho1a[5] * rho0a[1:4] * 2  # d_uu_u
    wva[1:4] += frgt[7] * rho1b[0] * rho1b[5] * rho0a[1:4] * 2  # d_uu_d
    wva[1:4] += frgt[8] * rho1b[0] * rho1a[5] * rho0b[1:4]      # d_ud_u
    wva[1:4] += frgt[9] * rho1b[0] * rho1b[5] * rho0b[1:4]      # d_ud_d
    wva[1:4] += fggt[0] * rho1a[5] * a0a1 * rho0a[1:4] * 8  # uu_uu_u
    wva[1:4] += fggt[1] * rho1b[5] * a0a1 * rho0a[1:4] * 8  # uu_uu_d
    wva[1:4] += fggt[2] * rho1a[5] * a0a1 * rho0b[1:4] * 4  # uu_ud_u
    wva[1:4] += fggt[2] * rho1a[5] * a0b1 * rho0a[1:4] * 4  # uu_ud_u
    wva[1:4] += fggt[2] * rho1a[5] * b0a1 * rho0a[1:4] * 4  # uu_ud_u
    wva[1:4] += fggt[3] * rho1b[5] * a0a1 * rho0b[1:4] * 4  # uu_ud_d
    wva[1:4] += fggt[3] * rho1b[5] * a0b1 * rho0a[1:4] * 4  # uu_ud_d
    wva[1:4] += fggt[3] * rho1b[5] * b0a1 * rho0a[1:4] * 4  # uu_ud_d
    wva[1:4] += fggt[4] * rho1a[5] * b0b1 * rho0a[1:4] * 8  # uu_dd_u
    wva[1:4] += fggt[5] * rho1b[5] * b0b1 * rho0a[1:4] * 8  # uu_dd_d
    wva[1:4] += fggt[6] * rho1a[5] * a0b1 * rho0b[1:4] * 2  # ud_ud_u
    wva[1:4] += fggt[6] * rho1a[5] * b0a1 * rho0b[1:4] * 2  # ud_ud_u
    wva[1:4] += fggt[7] * rho1b[5] * a0b1 * rho0b[1:4] * 2  # ud_ud_d
    wva[1:4] += fggt[7] * rho1b[5] * b0a1 * rho0b[1:4] * 2  # ud_ud_d
    wva[1:4] += fggt[8] * rho1a[5] * b0b1 * rho0b[1:4] * 4  # ud_dd_u
    wva[1:4] += fggt[9] * rho1b[5] * b0b1 * rho0b[1:4] * 4  # ud_dd_d
    wva[5] += frrt[0] * rho1a[0] * rho1a[0] * .5  # u_u_u
    wva[5] += frrt[2] * rho1a[0] * rho1b[0]       # u_d_u
    wva[5] += frrt[4] * rho1b[0] * rho1b[0] * .5  # d_d_u
    wva[5] += frtt[0] * rho1a[5] * rho1a[0]  # u_u_u
    wva[5] += frtt[1] * rho1b[5] * rho1a[0]  # u_u_d
    wva[5] += frtt[3] * rho1a[5] * rho1b[0]  # d_u_u
    wva[5] += frtt[4] * rho1b[5] * rho1b[0]  # d_u_d
    wva[5] += fgt[0] * a1a1  # uu_u
    wva[5] += fgt[2] * a1b1  # ud_u
    wva[5] += fgt[4] * b1b1  # dd_u
    wva[5] += frgt[0] * rho1a[0] * a0a1 * 2  # u_uu_u
    wva[5] += frgt[2] * rho1a[0] * a0b1      # u_ud_u
    wva[5] += frgt[2] * rho1a[0] * b0a1      # u_ud_u
    wva[5] += frgt[4] * rho1a[0] * b0b1 * 2  # u_dd_u
    wva[5] += frgt[6] * rho1b[0] * a0a1 * 2  # d_uu_u
    wva[5] += frgt[8] * rho1b[0] * a0b1      # d_ud_u
    wva[5] += frgt[8] * rho1b[0] * b0a1      # d_ud_u
    wva[5] += frgt[10]* rho1b[0] * b0b1 * 2  # d_dd_u
    wva[5] += fggt[0] * a0a1_a0a1 * 2          # uu_uu_u
    wva[5] += fggt[2] * a0a1_a0b1 * 2          # uu_ud_u
    wva[5] += fggt[2] * a0a1_b0a1 * 2          # uu_ud_u
    wva[5] += fggt[4] * a0a1_b0b1 * 4          # uu_dd_u
    wva[5] += fggt[6] * (a0b1 + b0a1)**2 * .5  # ud_ud_u
    wva[5] += fggt[8] * a0b1_b0b1 * 2          # ud_dd_u
    wva[5] += fggt[8] * b0a1_b0b1 * 2          # ud_dd_u
    wva[5] += fggt[10]* b0b1_b0b1 * 2          # dd_dd_u
    wva[5] += fgtt[0] * a0a1 * rho1a[5] * 2  # uu_u_u
    wva[5] += fgtt[1] * a0a1 * rho1b[5] * 2  # uu_u_d
    wva[5] += fgtt[3] * a0b1 * rho1a[5]      # ud_u_u
    wva[5] += fgtt[3] * b0a1 * rho1a[5]      # ud_u_u
    wva[5] += fgtt[4] * a0b1 * rho1b[5]      # ud_u_d
    wva[5] += fgtt[4] * b0a1 * rho1b[5]      # ud_u_d
    wva[5] += fgtt[6] * b0b1 * rho1a[5] * 2  # dd_u_u
    wva[5] += fgtt[7] * b0b1 * rho1b[5] * 2  # dd_u_d
    wva[5] += fttt[0] * rho1a[5] * rho1a[5] * .5  # u_u_u
    wva[5] += fttt[1] * rho1a[5] * rho1b[5]       # u_u_d
    wva[5] += fttt[2] * rho1b[5] * rho1b[5] * .5  # u_d_d
    wva *= weight
    wva[0] *= .5
    wva[5] *= .5
    ...
    wvb[0] += u_d_d * rho1b[0] * rho1a[0] * 2
    wvb[0] += u_u_d * rho1a[0] * rho1a[0]
    wvb[0] += frrt[2] * rho1a[0] * rho1a[5] * 2  # u_d_u
    wvb[0] += frrt[3] * rho1a[0] * rho1b[5] * 2  # u_d_d
    wvb[0] += frrt[4] * rho1b[0] * rho1a[5] * 2  # d_d_u
    wvb[0] += frrt[5] * rho1b[0] * rho1b[5] * 2  # d_d_d
    wvb[0] += d_dd * b1b1 * 2
    wvb[0] += d_ud * b1a1 * 2
    wvb[0] += d_uu * a1a1 * 2
    wvb[0] += d_d_dd * rho1b[0] * b0b1 * 4
    ...
    wvb[0] += d_ud_dd * b0a1_b0b1 * 4
    wvb[0] += d_uu_dd * b0b1_a0a1 * 8
    wvb[0] += d_ud_ud * (b0a1 + a0b1)**2
    wvb[0] += d_uu_ud * a0b1_a0a1 * 4
    wvb[0] += d_uu_ud * b0a1_a0a1 * 4
    wvb[0] += d_uu_uu * a0a1_a0a1 * 4
    wvb[0] += frtt[3] * rho1a[5] * rho1a[5]           # d_u_u
    wvb[0] += frtt[4] * rho1b[5] * rho1a[5] * 2       # d_u_d
    wvb[0] += frtt[5] * rho1b[5] * rho1b[5]           # d_d_d
    wvb[0] += frgt[6] * rho1a[5] * a0a1 * 4           # d_uu_u
    wvb[0] += frgt[7] * rho1b[5] * a0a1 * 4           # d_uu_d
    wvb[0] += frgt[8] * rho1a[5] * (b0a1 + a0b1) * 2  # d_ud_u
    wvb[0] += frgt[9] * rho1b[5] * (b0a1 + a0b1) * 2  # d_ud_d
    wvb[0] += frgt[10]* rho1a[5] * b0b1 * 4           # d_dd_u
    wvb[0] += frgt[11]* rho1b[5] * b0b1 * 4           # d_dd_d
    ...
    wvb[1:4] += d_d_ud * rho1b[0] * rho1b[0] * rho0a[1:4]
    wvb[1:4] += u_d_ud * rho1b[0] * rho1a[0] * rho0a[1:4] * 2
    wvb[1:4] += u_u_ud * rho1a[0] * rho1a[0] * rho0a[1:4]
    wvb[1:4] += frgt[2] * rho1a[5] * rho1a[0] * rho0a[1:4]      # u_ud_u
    wvb[1:4] += frgt[3] * rho1b[5] * rho1a[0] * rho0a[1:4]      # u_ud_d
    wvb[1:4] += frgt[4] * rho1a[5] * rho1a[0] * rho0b[1:4] * 2  # u_dd_u
    wvb[1:4] += frgt[5] * rho1b[5] * rho1a[0] * rho0b[1:4] * 2  # u_dd_d
    wvb[1:4] += frgt[8] * rho1a[5] * rho1b[0] * rho0a[1:4]      # d_ud_u
    wvb[1:4] += frgt[9] * rho1b[5] * rho1b[0] * rho0a[1:4]      # d_ud_d
    wvb[1:4] += frgt[10]* rho1a[5] * rho1b[0] * rho0b[1:4] * 2  # d_dd_u
    wvb[1:4] += frgt[11]* rho1b[5] * rho1b[0] * rho0b[1:4] * 2  # d_dd_d
    ...
    wvb[1:4] += ud_dd_dd * b0b1_b0a1 * rho0b[1:4] * 8
    wvb[1:4] += ud_dd_dd * b0b1_b0b1 * rho0a[1:4] * 4
    wvb[1:4] += uu_dd_dd * b0b1_a0a1 * rho0b[1:4] * 16
    wvb[1:4] += ud_ud_dd * (b0a1+a0b1)**2 * rho0b[1:4] * 2
    wvb[1:4] += ud_ud_dd * b0b1_a0b1 * rho0a[1:4] * 4
    wvb[1:4] += ud_ud_dd * b0b1_b0a1 * rho0a[1:4] * 4
    wvb[1:4] += uu_ud_dd * a0b1_a0a1 * rho0b[1:4] * 8
    wvb[1:4] += uu_ud_dd * b0a1_a0a1 * rho0b[1:4] * 8
    wvb[1:4] += uu_ud_dd * b0b1_a0a1 * rho0a[1:4] * 8
    wvb[1:4] += uu_uu_dd * a0a1_a0a1 * rho0b[1:4] * 8
    wvb[1:4] += ud_ud_ud * (b0a1+a0b1)**2 * rho0a[1:4]
    wvb[1:4] += uu_ud_ud * a0b1_a0a1 * rho0a[1:4] * 4
    wvb[1:4] += uu_ud_ud * b0a1_a0a1 * rho0a[1:4] * 4
    wvb[1:4] += uu_uu_ud * a0a1_a0a1 * rho0a[1:4] * 4
    wvb[1:4] += fgtt[3] * rho1a[5] * rho1a[5] * rho0a[1:4]      # ud_u_u
    wvb[1:4] += fgtt[4] * rho1b[5] * rho1a[5] * rho0a[1:4] * 2  # ud_u_d
    wvb[1:4] += fgtt[5] * rho1b[5] * rho1b[5] * rho0a[1:4]      # ud_d_d
    wvb[1:4] += fgtt[6] * rho1a[5] * rho1a[5] * rho0b[1:4] * 2  # dd_u_u
    wvb[1:4] += fgtt[7] * rho1b[5] * rho1a[5] * rho0b[1:4] * 4  # dd_u_d
    wvb[1:4] += fgtt[8] * rho1b[5] * rho1b[5] * rho0b[1:4] * 2  # dd_d_d
    wvb[1:4] += frgt[2] * rho1a[0] * rho1a[5] * rho0a[1:4]      # u_ud_u
    wvb[1:4] += frgt[3] * rho1a[0] * rho1b[5] * rho0a[1:4]      # u_ud_d
    wvb[1:4] += frgt[4] * rho1a[0] * rho1a[5] * rho0b[1:4] * 2  # u_dd_u
    wvb[1:4] += frgt[5] * rho1a[0] * rho1b[5] * rho0b[1:4] * 2  # u_dd_d
    wvb[1:4] += frgt[8] * rho1b[0] * rho1a[5] * rho0a[1:4]      # d_ud_u
    wvb[1:4] += frgt[9] * rho1b[0] * rho1b[5] * rho0a[1:4]      # d_ud_d
    wvb[1:4] += frgt[10]* rho1b[0] * rho1a[5] * rho0b[1:4] * 2  # d_dd_u
    wvb[1:4] += frgt[11]* rho1b[0] * rho1b[5] * rho0b[1:4] * 2  # d_dd_d
    wvb[1:4] += fggt[2] * rho1a[5] * a0a1 * rho0a[1:4] * 4  # uu_ud_u
    wvb[1:4] += fggt[3] * rho1b[5] * a0a1 * rho0a[1:4] * 4  # uu_ud_d
    wvb[1:4] += fggt[4] * rho1a[5] * a0a1 * rho0b[1:4] * 8  # uu_dd_u
    wvb[1:4] += fggt[5] * rho1b[5] * a0a1 * rho0b[1:4] * 8  # uu_dd_d
    wvb[1:4] += fggt[6] * rho1a[5] * a0b1 * rho0a[1:4] * 2  # ud_ud_u
    wvb[1:4] += fggt[6] * rho1a[5] * b0a1 * rho0a[1:4] * 2  # ud_ud_u
    wvb[1:4] += fggt[7] * rho1b[5] * a0b1 * rho0a[1:4] * 2  # ud_ud_d
    wvb[1:4] += fggt[7] * rho1b[5] * b0a1 * rho0a[1:4] * 2  # ud_ud_d
    wvb[1:4] += fggt[8] * rho1a[5] * a0b1 * rho0b[1:4] * 4  # ud_dd_u
    wvb[1:4] += fggt[8] * rho1a[5] * b0a1 * rho0b[1:4] * 4  # ud_dd_u
    wvb[1:4] += fggt[8] * rho1a[5] * b0b1 * rho0a[1:4] * 4  # ud_dd_u
    wvb[1:4] += fggt[9] * rho1b[5] * a0b1 * rho0b[1:4] * 4  # ud_dd_d
    wvb[1:4] += fggt[9] * rho1b[5] * b0a1 * rho0b[1:4] * 4  # ud_dd_d
    wvb[1:4] += fggt[9] * rho1b[5] * b0b1 * rho0a[1:4] * 4  # ud_dd_d
    wvb[1:4] += fggt[10]* rho1a[5] * b0b1 * rho0b[1:4] * 8  # dd_dd_u
    wvb[1:4] += fggt[11]* rho1b[5] * b0b1 * rho0b[1:4] * 8  # dd_dd_d
    wvb[5] += frrt[1] * rho1a[0] * rho1a[0] * .5  # u_u_d
    wvb[5] += frrt[3] * rho1b[0] * rho1a[0]       # u_d_d
    wvb[5] += frrt[5] * rho1b[0] * rho1b[0] * .5  # d_d_d
    wvb[5] += frtt[1] * rho1a[5] * rho1a[0]  # u_u_d
    wvb[5] += frtt[2] * rho1b[5] * rho1a[0]  # u_d_d
    wvb[5] += frtt[4] * rho1a[5] * rho1b[0]  # d_u_d
    wvb[5] += frtt[5] * rho1b[5] * rho1b[0]  # d_d_d
    wvb[5] += fgt[1] * a1a1  # uu_d
    wvb[5] += fgt[3] * b1a1  # ud_d
    wvb[5] += fgt[5] * b1b1  # dd_d
    wvb[5] += frgt[1] * rho1a[0] * a0a1 * 2  # u_uu_d
    wvb[5] += frgt[3] * rho1a[0] * a0b1      # u_ud_d
    wvb[5] += frgt[3] * rho1a[0] * b0a1      # u_ud_d
    wvb[5] += frgt[5] * rho1a[0] * b0b1 * 2  # u_dd_d
    wvb[5] += frgt[7] * rho1b[0] * a0a1 * 2  # d_uu_d
    wvb[5] += frgt[9] * rho1b[0] * a0b1      # d_ud_d
    wvb[5] += frgt[9] * rho1b[0] * b0a1      # d_ud_d
    wvb[5] += frgt[11]* rho1b[0] * b0b1 * 2  # d_dd_d
    wvb[5] += fggt[1] * a0a1_a0a1 * 2          # uu_uu_d
    wvb[5] += fggt[3] * a0b1_a0a1 * 2          # uu_ud_d
    wvb[5] += fggt[3] * b0a1_a0a1 * 2          # uu_ud_d
    wvb[5] += fggt[5] * b0b1_a0a1 * 4          # uu_dd_d
    wvb[5] += fggt[7] * (b0a1 + a0b1)**2 * .5  # ud_ud_d
    wvb[5] += fggt[9] * b0b1_a0b1 * 2          # ud_dd_d
    wvb[5] += fggt[9] * b0b1_b0a1 * 2          # ud_dd_d
    wvb[5] += fggt[11]* b0b1_b0b1 * 2          # dd_dd_d
    wvb[5] += fgtt[1] * a0a1 * rho1a[5] * 2  # uu_u_d
    wvb[5] += fgtt[2] * a0a1 * rho1b[5] * 2  # uu_d_d
    wvb[5] += fgtt[4] * a0b1 * rho1a[5]      # ud_u_d
    wvb[5] += fgtt[4] * b0a1 * rho1a[5]      # ud_u_d
    wvb[5] += fgtt[5] * a0b1 * rho1b[5]      # ud_d_d
    wvb[5] += fgtt[5] * b0a1 * rho1b[5]      # ud_d_d
    wvb[5] += fgtt[7] * b0b1 * rho1a[5] * 2  # dd_u_d
    wvb[5] += fgtt[8] * b0b1 * rho1b[5] * 2  # dd_d_d
    wvb[5] += fttt[1] * rho1a[5] * rho1a[5] * .5  # u_u_d
    wvb[5] += fttt[2] * rho1b[5] * rho1a[5]       # u_d_d
    wvb[5] += fttt[3] * rho1b[5] * rho1b[5] * .5  # d_d_d
    wvb *= weight
    wvb[0] *= .5
    wvb[5] *= .5
