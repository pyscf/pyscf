...
class KnowValues(unittest.TestCase):
  ...
  def test_140_h2o_uks_rpa_pb(self):
    """ This  """
    mol = gto.M(verbose=1,atom='O 0 0 0; H 0 0.489 1.074; H 0 0.489 -1.074',basis='cc-pvdz')
    gto_mf = dft.UKS(mol)
    gto_mf.kernel()
    nao_mf = tddft_iter(gto=mol, mf=gto_mf, xc_code="RPA")
    ...
    np.savetxt('test_140_h2o_uks_rpa_pb.txt', data.T, fmt=['%f','%f'])
    data_ref = np.loadtxt('test_140_h2o_uks_rpa_pb.txt-ref').T
