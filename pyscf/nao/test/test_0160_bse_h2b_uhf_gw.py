...
class KnowValues(unittest.TestCase):
  ...
  def test_160_bse_h2b_uhf_gw(self):
    """ This  """
    ...
    np.savetxt('test_0160_bse_h2b_uhf_cis_pyscf.txt', data.T, fmt=['%f','%f'])
    #data_ref = np.loadtxt('test_0160_bse_h2b_uhf_cis_pyscf.txt-ref').T
    #self.assertTrue(np.allclose(data_ref, data, atol=1e-6, rtol=1e-3))
    
    nao_td  = bse_iter(mf=gto_mf, gto=mol, verbosity=0, perform_gw=True, xc_code='GW')

    polariz = -nao_td.comp_polariz_inter_ave(omegas).imag
    data = np.array([omegas.real*HARTREE2EV, polariz])
    np.savetxt('test_0160_bse_h2b_uhf_gw_nao.txt', data.T, fmt=['%f','%f'])
    data_ref = np.loadtxt('test_0160_bse_h2b_uhf_gw_nao.txt-ref').T
    self.assertTrue(np.allclose(data_ref, data, atol=1e-6, rtol=1e-3))
