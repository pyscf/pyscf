...
class gw_iter(gw):
  ...
  def si_c2(self,ww):
    """
    This computes the correlation part of the screened interaction using
    LinearOpt and lgmres
  ...
  def si_c_check (self, tol = 1e-5):
    """
    This compares np.solve and LinearOpt-lgmres methods for solving linear
    equation (1-v\chi_{0}) * W_c = v\chi_{0}v
    """
    ...
    t2 = time.time()
    # method 2:  scipy.sparse.linalg.lgmres
    si0_2 = self.si_c2(ww)
    t3 = time.time() - t2
  ...
  def get_snmw2sf_iter(self, optimize="greedy"):
    ...
    for s in range(self.nspin):
        ...
        for iw, w in enumerate(ww):
            ...
            inm[:, :, iw] = np.einsum('nmp,nmp->nm',xvx[s], sf_aux,
                                      optimize=optimize)
  ...
  def gw_chi0_mv(self,dvin, comega=1j*0.0):
    ...
    for s in range(self.nspin):
        ...
        if self.use_numba:
            self.div_numba(self.ksn2e[0, s], self.ksn2f[0,s], nf, vs, comega,
                           nm2v_re, nm2v_im)
        else:
    ...
    for s in range(self.nspin):
      ...
      for iw,w in enumerate(ww):     
          ...
          for n in range (len(self.nn[s])):
              ...
              v_eff[n,:]=np.solve(k_c, xvxbxvx[n,:])
  ...
  def g0w0_eigvals_iter(self):
    ...
    for i in range(self.niter_max_ev):
      ...
      sn2eval_gw = []
      for s,(evhf,n2i,n2r,nn) in enumerate(zip(self.h0_vh_x_expval,
                                               sn2i,sn2r,self.nn)):
