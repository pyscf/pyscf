...
class KnownValues(unittest.TestCase):
    def setUp(self):
        dx = 0.0001
        self.mol0 = gto.M(atom='H 0 0 0; H 0 1 1.2; H 1. .1 0; H .5 .5 1', unit='B')
        self.mol1 = gto.M(atom='H 0 0 %g; H 0 1 1.2; H 1. .1 0; H .5 .5 1'%(-dx), unit='B')
        self.mol2 = gto.M(atom='H 0 0 %g; H 0 1 1.2; H 1. .1 0; H .5 .5 1'%dx, unit='B')
        self.dx = dx * 2
        nao = self.mol0.nao_nr()
    ...
    def test_e_psi1(self):
        def get_e_psi1(pcmobj):
            ...
            phi = ddcosmo.make_phi(pcmobj, self.dm, r_vdw, ui)
            L_X = numpy.linalg.solve(Lmat, phi.ravel()).reshape(natm,-1)
            psi, vmat, L_S = \
                    ddcosmo.make_psi_vmat(pcmobj, self.dm, r_vdw, ui, pcmobj.grids, ylm_1sph,
                                          cached_pol, L_X, Lmat)
            psi1 = ddcosmo_grad.make_e_psi1(pcmobj, self.dm, r_vdw, ui, pcmobj.grids, ylm_1sph,
                                            cached_pol, L_X, Lmat)
            return L_X, psi, psi1

        pcmobj = ddcosmo.DDCOSMO(self.mol0)
        L_X, psi0, psi1 = get_e_psi1(pcmobj)

        pcmobj = ddcosmo.DDCOSMO(self.mol1)
        L_X1, psi = get_e_psi1(pcmobj)[:2]
        e1 = numpy.einsum('jx,jx', psi, L_X)

        pcmobj = ddcosmo.DDCOSMO(self.mol2)
        L_X2, psi = get_e_psi1(pcmobj)[:2]
        e2 = numpy.einsum('jx,jx', psi, L_X)
        self.assertAlmostEqual(abs((e2-e1)/self.dx - psi1[0,2]).max(), 0, 7)
    ...
    def test_phi(self):
        def get_phi1(pcmojb):
            ...
            psi, vmat, L_S = \
                    ddcosmo.make_psi_vmat(pcmobj, self.dm, r_vdw, ui, pcmobj.grids, ylm_1sph,
                                          cached_pol, L_X, Lmat)
            phi1 = ddcosmo_grad.make_phi1(pcmobj, self.dm, r_vdw, ui)
        ...
        self.assertAlmostEqual(abs((e2-e1)/self.dx - phi1[0,2]).max(), 0, 7)
    ...
    def test_fi(self):
        ...
        ui_2 = 1 - fi_2
        ui_2[ui_2<0] = 0
        self.assertAlmostEqual(abs((fi_2-fi_1)/self.dx - fi1[0,2]).max(), 0, 6)
        self.assertAlmostEqual(abs((ui_2-ui_1)/self.dx - ui1[0,2]).max(), 0, 6)
    ...
    def test_L1(self):
        ...
        self.assertAlmostEqual(abs((L_2-L_1)/self.dx - L1[0,2]).max(), 0, 7)

    def test_e_cosmo_grad(self):
        pcmobj = ddcosmo.DDCOSMO(self.mol0)
        de = ddcosmo_grad.kernel(pcmobj, self.dm)
        pcmobj = ddcosmo.DDCOSMO(self.mol1)
        e1 = pcmobj.energy(self.dm)
        pcmobj = ddcosmo.DDCOSMO(self.mol2)
        e2 = pcmobj.energy(self.dm)
        self.assertAlmostEqual(abs((e2-e1)/self.dx - de[0,2]).max(), 0, 7)

    def test_scf_grad(self):
        mf = ddcosmo.ddcosmo_for_scf(scf.RHF(self.mol0)).run()
        de_cosmo = ddcosmo_grad.kernel(mf._solvent, mf.make_rdm1())
        de = mf.nuc_grad_method().kernel()
        dm1 = mf.make_rdm1()

        mf = ddcosmo.ddcosmo_for_scf(scf.RHF(self.mol1)).run()
        e1 = mf.e_tot
        e1_cosmo = mf._solvent.energy(dm1)

        mf = ddcosmo.ddcosmo_for_scf(scf.RHF(self.mol2)).run()
        e2 = mf.e_tot
        e2_cosmo = mf._solvent.energy(dm1)
        self.assertAlmostEqual(abs((e2-e1)/self.dx - de[0,2]).max(), 0, 7)
        self.assertAlmostEqual(abs((e2_cosmo-e1_cosmo)/self.dx - de_cosmo[0,2]).max(), 0, 7)
