...
from pyscf.ctfcc import kccsd_uhf, eom_kccsd_uhf
...
myip = eom_kccsd_uhf.EOMIP(mycc)
myea = eom_kccsd_uhf.EOMEA(mycc)

refip = cc.eom_kccsd_uhf.EOMIP(refcc)
refea = cc.eom_kccsd_uhf.EOMEA(refcc)
...
class REFTEST(unittest.TestCase):
    ...
    def test_amp_vec_ip(self):
        ...
        for i, j in zip(r11+r21, r11r+r21r):
            self.assertTrue(numpy.linalg.norm(i.array.to_nparray()-j)<1e-10)
    ...
    def test_matvec_ip(self):
        ...
        for i, j in zip(r11+r21, r11r+r21r):
            self.assertTrue(numpy.linalg.norm(i.array.to_nparray()-j)<1e-6)
