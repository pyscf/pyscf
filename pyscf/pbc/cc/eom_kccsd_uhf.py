...
def eaccsd_matvec(eom, vector, kshift, imds=None, diag=None):
    ...
    #        tmp2aa = lib.einsum('ldbj,lad->jab', imds.Wovvo[kl, kd, kb], r2aaa[kl, ka])
    #        tmp2aa += lib.einsum('ldbj,lad->jab', imds.WOVvo[kl, kd, kb], r2bab[kl, ka])
    #        Hr2aaa[kj, ka] += tmp2aa - tmp2aaT

    #        Hr2bab[kj, ka] += lib.einsum('ldbj,lad->jab', imds.WovVO[kl, kd, kb], r2aaa[kl, ka])
    #        Hr2bab[kj, ka] += lib.einsum('ldbj,lad->jab', imds.WOVVO[kl, kd, kb], r2bab[kl, ka])
    #        Hr2bab[kj, ka] += lib.einsum('ldaj,ldb->jab', imds.WOvvO[kl, kd, ka], r2bab[kl, kb])

    #        Hr2aba[kj, ka] += lib.einsum('ldbj,lad->jab', imds.WOVvo[kl, kd, kb], r2bbb[kl, ka])
    #        Hr2aba[kj, ka] += lib.einsum('ldbj,lad->jab', imds.Wovvo[kl, kd, kb], r2aba[kl, ka])
    #        Hr2aba[kj, ka] += lib.einsum('ldaj,ldb->jab', imds.WoVVo[kl, kd, ka], r2aba[kl, kb])

    #        tmp2bb = lib.einsum('ldbj,lad->jab', imds.WOVVO, r2bbb)
    #        tmp2bb += lib.einsum('ldbj,lad->jab', imds.WovVO, r2aba)
    #        Hr2bbb[kj, ka] += tmp2bb - tmp2bbT
...
class _IMDS:
    ...
    def make_ea(self):
        ...
        #self.Wvvvo, self.WvvVO, self.WVVvo, self.WVVVO = kintermediates_uhf.Wvvvo(t1, t2, eris, Wvvvv)  # TODO
