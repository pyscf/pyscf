...
from pyscf.pbc.gw.kuqsgw import KUQSGW
...
def test_kuqsgw_with_fc(hydrogen_kuhf):
    ...
    assert gw.mo_energy[0][0][1] == pytest.approx(-0.4685377, abs=5e-4)
    assert gw.mo_energy[0][0][2] == pytest.approx(0.12077409, abs=5e-4)
    assert gw.mo_energy[1][1][0] == pytest.approx(-0.5248676, abs=5e-4)
    assert gw.mo_energy[1][1][1] == pytest.approx(0.06330342, abs=5e-4)
