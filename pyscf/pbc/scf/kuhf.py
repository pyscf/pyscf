...
def _get_init_guess(kmf, cell=None, key='minao'):
    if cell is None:
        cell = kmf.cell
    dm_kpts = None
    if key.lower() == '1e':
        dm_kpts = kmf.init_guess_by_1e(cell)
    elif getattr(cell, 'natm', 0) == 0:
        logger.info(kmf, 'No atom found in cell. Use 1e initial guess')
        dm_kpts = kmf.init_guess_by_1e(cell)
    elif key.lower() == 'atom':
        dm = kmf.init_guess_by_atom(cell)
    elif key.lower().startswith('chk'):
        try:
            dm_kpts = kmf.from_chk()
        except (IOError, KeyError):
            logger.warn(kmf, 'Fail to read %s. Use MINAO initial guess',
                        kmf.chkfile)
            dm = kmf.init_guess_by_minao(cell)
    else:
        dm = kmf.init_guess_by_minao(cell)
    ...
    if cell.dimension < 3:
        ne = np.einsum('xkij,kji->xk', dm_kpts, kmf.get_ovlp(cell))
        nelec = np.asarray(cell.nelec).reshape(2,1)
        if np.any(abs(ne - nelec) > 1e-7):
            logger.warn(kmf, 'Big error detected in the electron number '
...
class KUHF(pbcuhf.UHF, khf.KSCF):
    ...
    get_init_guess = _get_init_guess
