...
THC_INDICES_LIST = ["P", "Q", "R", "S", "T", "U", "V", "W"]
...
def thc_einsum(subscripts, *tensors, **kwargs):
    ...
    if len(tensors) <= 1 or '...' in subscripts:
        ...
        #### THE FOLLOWING CODE SHOULD SWITCH TO COTENGRA ####
...
if __name__ == "__main__":
    ...
    mp2_J = thc_einsum("iajb,iajb,ijab->", eri, eri, ene_deno, optimize='greedy')
    mp2_K = thc_einsum("iajb,ibja,ijab->", eri, eri, ene_deno, optimize='greedy')
    
    print(-2*mp2_J+mp2_K)
    
    mp3_CX_1 = thc_einsum("iajb,jkbc,iakc,ijab,ikac->", eri, eri, eri, ene_deno, ene_deno)