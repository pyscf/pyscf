        ...
        for ki in range(nkpts):
            for kk in range(nkpts):
                for kc in range(nkpts):
                    ...
                    if isinstance(eris.ovvv, type(None)):
                        ...
                        for p in range(0,nocc,chnk_size):
                             ...
                             k = eris_ovvv.shape[0]
                             t1_2[ki] += 1.5*lib.einsum('kdac,ikcd->ia',eris_ovvv,t2_1[ki,kk,kc,:,a:a+k],optimize=True)
                             t1_2[ki] -= 0.5*lib.einsum('kdac,kicd->ia',eris_ovvv,t2_1[kk,ki,kc,a:a+k,:],optimize=True)
                             del eris_ovvv
                             eris_ovvv = dfadc.get_ovvv_df(myadc, eris.Lov[kk,kc], eris.Lvv[ka,kd], p, chnk_size).reshape(-1,nvir,nvir,nvir)/nkpts
                             t1_2[ki] -= 0.5*lib.einsum('kcad,ikcd->ia',eris_ovvv,t2_1[ki,kk,kc,:,a:a+k],optimize=True)
                             t1_2[ki] += 0.5*lib.einsum('kcad,kicd->ia',eris_ovvv,t2_1[kk,ki,kc,a:a+k,:],optimize=True)
                             del eris_ovvv
                             a += k
                    else:
                        eris_ovvv = eris.ovvv[:]
                        t1_2[ki] += 1.5*lib.einsum('kdac,ikcd->ia',eris_ovvv[kk,kd,ka],t2_1[ki,kk,kc],optimize=True)
                        t1_2[ki] -= 0.5*lib.einsum('kdac,kicd->ia',eris_ovvv[kk,kd,ka],t2_1[kk,ki,kc],optimize=True)
                        t1_2[ki] -= 0.5*lib.einsum('kcad,ikcd->ia',eris_ovvv[kk,kc,ka],t2_1[ki,kk,kc],optimize=True)
                        t1_2[ki] += 0.5*lib.einsum('kcad,kicd->ia',eris_ovvv[kk,kc,ka],t2_1[kk,ki,kc],optimize=True)
                        del eris_ovvv
                ...
                for kl in range(nkpts):
                    ...
                    t1_2[ki] -= 1.5*lib.einsum('lcki,klac->ia',eris_ovoo[kl,kc,kk],t2_1[kk,kl,ka],optimize=True)
                    t1_2[ki] += 0.5*lib.einsum('lcki,lkac->ia',eris_ovoo[kl,kc,kk],t2_1[kl,kk,ka],optimize=True)
                    t1_2[ki] -= 0.5*lib.einsum('kcli,lkac->ia',eris_ovoo[kk,kc,kl],t2_1[kl,kk,ka],optimize=True)
                    t1_2[ki] += 0.5*lib.einsum('kcli,klac->ia',eris_ovoo[kk,kc,kl],t2_1[kk,kl,ka],optimize=True)
    ...
    # Compute second-order doubles t2 (tijab)
        ...
        for ki, kj, ka in kpts_helper.loop_kkk(nkpts):
            for kk in range(nkpts):
                    ...
                    t2_2[ki,kj,ka] -= lib.einsum('kjbc,kica->ijab',eris_oovv[kk,kj,kb],t2_1[kk,ki,kc],optimize=True)

                    kc = kconserv[kk,ka,kj]
                    kb = kconserv[ki,kk,kc]
                    t2_2[ki,kj,ka] -= lib.einsum('kibc,jkca->ijab',eris_oovv[kk,ki,kb],t2_1[kj,kk,kc],optimize=True)

                    kc = kconserv[ka,kj,kk]
                    kb = kconserv[ki,kc,kk]
                    t2_2[ki,kj,ka] -= lib.einsum('kjac,ikcb->ijab',eris_oovv[kk,kj,ka],t2_1[ki,kk,kc],optimize=True)

                    kc = kconserv[ka,ki,kk]
                    kb = kconserv[kk,kc,kj]
                    t2_2[ki,kj,ka] -= lib.einsum('kiac,kjcb->ijab',eris_oovv[kk,ki,ka],t2_1[kk,kj,kc],optimize=True)

            for kl in range(nkpts):
                    kk = kconserv[kj,kl,ki]
                    t2_2[ki,kj,ka] += lib.einsum('kilj,klab->ijab',eris_oooo[kk,ki,kl],t2_1[kk,kl,ka],optimize=True)
...
def contract_ladder(myadc,t_amp,vvvv,ka,kb,kc):
    ...
    if isinstance(vvvv, np.ndarray):
        ...
        for p in range(0,nvir,chnk_size):
            ...
            k = vvvv_p.shape[0]
            t += np.dot(t_amp[:,a:a+k].conj(),vvvv_p).reshape(nocc,nocc,nvir,nvir)
            del vvvv_p
            a += k
    else :
        for p in range(0,nvir,chnk_size):
    ...
    if(method =='adc(3)'):
           ...
           for kl in range(nkpts):
               ...
               if isinstance(eris.ovvv, type(None)):
                   ...
                   for p in range(0,nocc,chnk_size):
                       ...
                       k = eris_ovvv.shape[0]
                       M_ab[ka] += 2. * lib.einsum('ld,ldab->ab',t1_2[kl][a:a+k], eris_ovvv,optimize=True)
                       del eris_ovvv
                       ...
                       k = eris_ovvv.shape[0]
                       M_ab[ka] += 2. * lib.einsum('ld,ldba->ab',t1_2[kl][a:a+k].conj(), eris_ovvv.conj(),optimize=True)
                       del eris_ovvv
                       kd = kconserv[ka,kb,kl]
                       eris_ovvv = dfadc.get_ovvv_df(adc, eris.Lov[kl,kb], eris.Lvv[ka,kd], p, chnk_size).reshape(-1,nvir,nvir,nvir)/nkpts
                       k = eris_ovvv.shape[0]
                       M_ab[ka] -= lib.einsum('ld,lbad->ab',t1_2[kl][a:a+k], eris_ovvv,optimize=True)
                       del eris_ovvv
                       kd = kconserv[kb,ka,kl]
                       eris_ovvv = dfadc.get_ovvv_df(adc, eris.Lov[kl,ka], eris.Lvv[kb,kd], p, chnk_size).reshape(-1,nvir,nvir,nvir)/nkpts
                       k = eris_ovvv.shape[0]
                       M_ab[ka] -= lib.einsum('ld,labd->ab',t1_2[kl,a:a+k].conj(), eris_ovvv.conj(),optimize=True)
                       del eris_ovvv
                       a += k
                   ...
                   eris_ovvv = eris.ovvv
                   kd = kconserv[ka,kb,kl]
                   M_ab[ka] += 2. * lib.einsum('ld,ldab->ab',t1_2[kl], eris_ovvv[kl,kd,ka],optimize=True)
                   M_ab[ka] -=  lib.einsum('ld,lbad->ab',t1_2[kl], eris_ovvv[kl,kb,ka],optimize=True)
                   kd = kconserv[kb,ka,kl]
                   M_ab[ka] += 2. * lib.einsum('ld,ldba->ab',t1_2[kl].conj(), eris_ovvv[kl,kd,kb].conj(),optimize=True)
                   M_ab[ka] -= lib.einsum('ld,labd->ab',t1_2[kl].conj(), eris_ovvv[kl,ka,kb].conj(),optimize=True)
       ...
       for ka in range(nkpts):
           ...
           for kn, ke, kd in kpts_helper.loop_kkk(nkpts):
               ...
               M_ab[ka] += 0.5 *2. * lib.einsum('nemb,neam->ab',temp_t2_v_1, eris_ovvo[kn,ke,ka], optimize=True)
               M_ab[ka] += 0.5 *2. * lib.einsum('nema,nebm->ab',temp_t2_v_1, eris_ovvo[kn,ke,kb], optimize=True)
               ...
               M_ab[ka] += 0.5 *2. * lib.einsum('mbne,neam->ab',temp_t2_v_1, eris_ovvo[kn,ke,ka], optimize=True)
               M_ab[ka] -= 0.5 *     lib.einsum('mane,nmbe->ab',temp_t2_v_1, eris_oovv[kn,km,kb], optimize=True)
               M_ab[ka] += 0.5 *2. * lib.einsum('mane,nebm->ab',temp_t2_v_1, eris_ovvo[kn,ke,kb], optimize=True)
               del temp_t2_v_1
               ...
               M_ab[ka] -= 0.5 * 4. * lib.einsum('nemb,neam->ab',temp_t2_v_2, eris_ovvo[kn,ke,ka], optimize=True)
               M_ab[ka] += 0.5 * 2. * lib.einsum('nema,nmbe->ab',temp_t2_v_2, eris_oovv[kn,km,kb], optimize=True)
               M_ab[ka] -= 0.5 * 4. * lib.einsum('nema,nebm->ab',temp_t2_v_2, eris_ovvo[kn,ke,kb], optimize=True)
               del temp_t2_v_2

               temp_t2_v_3 = lib.einsum('lned,lmbd->nemb',t2_1[kl,kn,ke], t2_1[kl,km,kb].conj(),optimize=True)
               M_ab[ka] -= 0.5 *      lib.einsum('nemb,neam->ab',temp_t2_v_3, eris_ovvo[kn,ke,ka], optimize=True)
               ...
               M_ab[ka] -= 0.5 *      lib.einsum('nema,nebm->ab',temp_t2_v_3, eris_ovvo[kn,ke,kb], optimize=True)
               del temp_t2_v_3
           ...
           for km in range(nkpts):
               for kl in range(nkpts):
                   ...
                   t2_1_mla = adc.t2[0][km,kl,ka]
                   M_ab[ka] -= 0.5 * 0.25*lib.einsum('mlaf,mlbf->ab',t2_1_mla, temp_t2[km,kl,kb], optimize=True)
                   M_ab[ka] += 0.5 * 0.25*lib.einsum('mlaf,lmbf->ab',t2_1_mla, temp_t2[kl,km,kb], optimize=True)
                   M_ab[ka] += 0.5 * 0.25*lib.einsum('mlaf,mlfb->ab',t2_1_mla, temp_t2[km,kl,kf], optimize=True)
                   M_ab[ka] -= 0.5 * 0.25*lib.einsum('mlaf,lmfb->ab',t2_1_mla, temp_t2[kl,km,kf], optimize=True)
                   M_ab[ka] -= 0.5 *      lib.einsum('mlaf,mlbf->ab',t2_1_mla, temp_t2[km,kl,kb], optimize=True)
                   del t2_1_mla

                   t2_1_lma = adc.t2[0][kl,km,ka]
                   M_ab[ka] += 0.5 * 0.25*lib.einsum('lmaf,mlbf->ab',t2_1_lma, temp_t2[km,kl,kb], optimize=True)
                   M_ab[ka] -= 0.5 * 0.25*lib.einsum('lmaf,lmbf->ab',t2_1_lma, temp_t2[kl,km,kb], optimize=True)
                   M_ab[ka] -= 0.5 * 0.25*lib.einsum('lmaf,mlfb->ab',t2_1_lma, temp_t2[km,kl,kf], optimize=True)
                   M_ab[ka] += 0.5 * 0.25*lib.einsum('lmaf,lmfb->ab',t2_1_lma, temp_t2[kl,km,kf], optimize=True)
                   del t2_1_lma

                   kd = kconserv[km,ka,kl]
                   t2_1_mlb = adc.t2[0][km,kl,kb]
                   M_ab[ka] -= 0.5 * 0.25*lib.einsum('mlad,mlbd->ab', temp_t2[km,kl,ka], t2_1_mlb, optimize=True)
                   M_ab[ka] += 0.5 * 0.25*lib.einsum('lmad,mlbd->ab', temp_t2[kl,km,ka], t2_1_mlb, optimize=True)
                   M_ab[ka] -= 0.5 *      lib.einsum('mlad,mlbd->ab', temp_t2[km,kl,ka], t2_1_mlb, optimize=True)
                   ...
                   t2_1_lmb = adc.t2[0][kl,km,kb]
                   M_ab[ka] += 0.5 * 0.25*lib.einsum('mlad,lmbd->ab', temp_t2[km,kl,ka], t2_1_lmb, optimize=True)
                   M_ab[ka] -= 0.5 * 0.25*lib.einsum('lmad,lmbd->ab', temp_t2[kl,km,ka], t2_1_lmb, optimize=True)
                   ...
                   for kd in range(nkpts):
                       ...
                       if isinstance(eris.vvvv, type(None)):
                           ...
                           a = 0
                           for p in range(0,nvir,chnk_size):
                               eris_vvvv = dfadc.get_vvvv_df(adc, eris.Lvv[kb,ka], eris.Lvv[ke,kf], p, chnk_size)/nkpts
                               k = eris_vvvv.shape[0]
                               M_ab[ka,a:a+k] -= lib.einsum('mldf,mled,beaf->ab',t2_1[km,kl,kd], t2_1[km,kl,ke].conj(), eris_vvvv, optimize=True)
                               M_ab[ka,a:a+k] += lib.einsum('mldf,lmed,beaf->ab',t2_1[km,kl,kd], t2_1[kl,km,ke].conj(), eris_vvvv, optimize=True)
                               M_ab[ka,a:a+k] += lib.einsum('lmdf,mled,beaf->ab',t2_1[kl,km,kd], t2_1[km,kl,ke].conj(), eris_vvvv, optimize=True)
                               M_ab[ka,a:a+k] -= lib.einsum('lmdf,lmed,beaf->ab',t2_1[kl,km,kd], t2_1[kl,km,ke].conj(), eris_vvvv, optimize=True)
                               M_ab[ka,a:a+k] += 2.*lib.einsum('mlfd,mled,beaf->ab',t2_1[km,kl,kf], t2_1[km,kl,ke].conj(), eris_vvvv, optimize=True)
                               del eris_vvvv

                               eris_vvvv = dfadc.get_vvvv_df(adc, eris.Lvv[kb,kf], eris.Lvv[ke,ka], p, chnk_size)/nkpts
                               M_ab[ka,a:a+k] += 0.5*lib.einsum('mldf,mled,befa->ab',t2_1[km,kl,kd], t2_1[km,kl,ke].conj(), eris_vvvv, optimize=True)
                               M_ab[ka,a:a+k] -= 0.5*lib.einsum('mldf,lmed,befa->ab',t2_1[km,kl,kd], t2_1[kl,km,ke].conj(), eris_vvvv, optimize=True)
                               M_ab[ka,a:a+k] -= 0.5*lib.einsum('lmdf,mled,befa->ab',t2_1[kl,km,kd], t2_1[km,kl,ke].conj(), eris_vvvv, optimize=True)
                               M_ab[ka,a:a+k] += 0.5*lib.einsum('lmdf,lmed,befa->ab',t2_1[kl,km,kd], t2_1[kl,km,ke].conj(), eris_vvvv, optimize=True)
                               M_ab[ka,a:a+k] -= lib.einsum('mlfd,mled,befa->ab',t2_1[km,kl,kf], t2_1[km,kl,ke].conj(), eris_vvvv, optimize=True)
                               del eris_vvvv
                               a += k

                       elif isinstance(eris.vvvv, np.ndarray):
                           eris_vvvv =  eris.vvvv
                           M_ab[ka] -= lib.einsum('mldf,mled,beaf->ab',t2_1[km,kl,kd], t2_1[km,kl,ke].conj(), eris_vvvv[ka,ke,kb], optimize=True)
                           M_ab[ka] += lib.einsum('mldf,lmed,beaf->ab',t2_1[km,kl,kd], t2_1[kl,km,ke].conj(), eris_vvvv[ka,ke,kb], optimize=True)
                           M_ab[ka] += lib.einsum('lmdf,mled,beaf->ab',t2_1[kl,km,kd], t2_1[km,kl,ke].conj(), eris_vvvv[ka,ke,kb], optimize=True)
                           M_ab[ka] -= lib.einsum('lmdf,lmed,beaf->ab',t2_1[kl,km,kd], t2_1[kl,km,ke].conj(), eris_vvvv[ka,ke,kb], optimize=True)
                           M_ab[ka] += 2.*lib.einsum('mlfd,mled,beaf->ab',t2_1[km,kl,kf], t2_1[km,kl,ke].conj(), eris_vvvv[ka,ke,kb], optimize=True)
                           M_ab[ka] += 0.5*lib.einsum('mldf,mled,befa->ab',t2_1[km,kl,kd], t2_1[km,kl,ke].conj(), eris_vvvv[ka,ke,kf], optimize=True)
                           M_ab[ka] -= 0.5*lib.einsum('mldf,lmed,befa->ab',t2_1[km,kl,kd], t2_1[kl,km,ke].conj(), eris_vvvv[ka,ke,kf], optimize=True)
                           M_ab[ka] -= 0.5*lib.einsum('lmdf,mled,befa->ab',t2_1[kl,km,kd], t2_1[km,kl,ke].conj(), eris_vvvv[ka,ke,kf], optimize=True)
                           M_ab[ka] += 0.5*lib.einsum('lmdf,lmed,befa->ab',t2_1[kl,km,kd], t2_1[kl,km,ke].conj(), eris_vvvv[ka,ke,kf], optimize=True)
                           M_ab[ka] -= lib.einsum('mlfd,mled,befa->ab',t2_1[km,kl,kf], t2_1[km,kl,ke].conj(), eris_vvvv[ka,ke,kf], optimize=True)
                       else :
                           chnk_size = adc.chnk_size
                           if chnk_size > nvir:
                               chnk_size = nvir
    ...
    for ki in range(nkpts):
        ...
        for kl in range(nkpts):
            for kd in range(nkpts):
                ...
                M_ij[ki] -= 0.5 * 0.5 *  lib.einsum('jlde,ield->ij',t2_1_jld.conj(), eris_ovov[ki,ke,kl].conj(),optimize=True)
                ...
                M_ij[ki] += 0.5 * 0.5 *  lib.einsum('ljde,ield->ij',t2_1_ljd.conj(), eris_ovov[ki,ke,kl].conj(),optimize=True)
                del t2_1_ljd
                del t2_1    
    ...
    if (method == "adc(3)"):
        ...
        for ki in range(nkpts):
            ...
            for kl in range(nkpts):
                ...
                M_ij[ki] += 2. * lib.einsum('ld,ldji->ij',t1_2[kl], eris_ovoo[kl,kd,kj],optimize=True)
                M_ij[ki] -=      lib.einsum('ld,jdli->ij',t1_2[kl], eris_ovoo[kj,kd,kl],optimize=True)

                kd = kconserv[ki,kj,kl]
                M_ij[ki] += 2. * lib.einsum('ld,ldij->ij',t1_2[kl].conj(), eris_ovoo[kl,kd,ki].conj(),optimize=True)
                M_ij[ki] -=      lib.einsum('ld,idlj->ij',t1_2[kl].conj(), eris_ovoo[ki,kd,kl].conj(),optimize=True)
            ...
            for km, ke, kd in kpts_helper.loop_kkk(nkpts):
                ...
                temp_t2_v_1 = lib.einsum('lmde,jldf->mejf',t2_1[kl,km,kd].conj(), t2_1[kj,kl,kd],optimize=True)
                M_ij[ki] -=  0.5 * 2 * lib.einsum('mejf,ifem->ij',temp_t2_v_1, eris_ovvo[ki,kf,ke],optimize = True)
                M_ij[ki] -=  0.5 * 2 * lib.einsum('meif,jfem->ij',temp_t2_v_1.conj(), eris_ovvo[kj,kf,ke].conj(),optimize = True)
                M_ij[ki] +=  0.5 *     lib.einsum('mejf,imef->ij',temp_t2_v_1, eris_oovv[ki,km,ke],optimize = True)
                M_ij[ki] +=  0.5 *     lib.einsum('meif,jmef->ij',temp_t2_v_1.conj(), eris_oovv[kj,km,ke].conj(),optimize = True)

                temp_t2_v_new = lib.einsum('mlde,ljdf->mejf',t2_1[km,kl,kd].conj(), t2_1[kl,kj,kd],optimize=True)
                M_ij[ki] -=  0.5 * 2 * lib.einsum('mejf,ifem->ij',temp_t2_v_new, eris_ovvo[ki,kf,ke],optimize = True)
                M_ij[ki] +=  0.5 *     lib.einsum('mejf,imef->ij',temp_t2_v_new, eris_oovv[ki,km,ke],optimize = True)
                M_ij[ki] -=  0.5 * 2 * lib.einsum('meif,jfem->ij',temp_t2_v_new.conj(), eris_ovvo[kj,kf,ke].conj(),optimize = True)
                M_ij[ki] +=  0.5 *     lib.einsum('meif,jmef->ij',temp_t2_v_new.conj(), eris_oovv[kj,km,ke].conj(),optimize = True)
                del temp_t2_v_new

                temp_t2_v_2 = lib.einsum('lmde,ljdf->mejf',t2_1[kl,km,kd].conj(), t2_1[kl,kj,kd],optimize=True)
                M_ij[ki] +=  0.5 * 4 * lib.einsum('mejf,ifem->ij',temp_t2_v_2, eris_ovvo[ki,kf,ke],optimize = True)
                M_ij[ki] +=  0.5 * 4 * lib.einsum('meif,jfem->ij',temp_t2_v_2.conj(), eris_ovvo[kj,kf,ke].conj(),optimize = True)
                M_ij[ki] -=  0.5 * 2 * lib.einsum('meif,jmef->ij',temp_t2_v_2.conj(), eris_oovv[kj,km,ke].conj(),optimize = True)
                M_ij[ki] -=  0.5 * 2 * lib.einsum('mejf,imef->ij',temp_t2_v_2, eris_oovv[ki,km,ke],optimize = True)
                del temp_t2_v_2

                temp_t2_v_3 = lib.einsum('mlde,jldf->mejf',t2_1[km,kl,kd].conj(), t2_1[kj,kl,kd],optimize=True)
                M_ij[ki] += 0.5 *    lib.einsum('mejf,ifem->ij',temp_t2_v_3, eris_ovvo[ki,kf,ke],optimize = True)
                M_ij[ki] += 0.5 *    lib.einsum('meif,jfem->ij',temp_t2_v_3.conj(), eris_ovvo[kj,kf,ke].conj(),optimize = True)
                M_ij[ki] -= 0.5 *2 * lib.einsum('meif,jmef->ij',temp_t2_v_3.conj(), eris_oovv[kj,km,ke].conj(),optimize = True)
                M_ij[ki] -= 0.5 *2 * lib.einsum('mejf,imef->ij',temp_t2_v_3, eris_oovv[ki,km,ke],optimize = True)
                del temp_t2_v_3
            ...
            for km, ke, kd in kpts_helper.loop_kkk(nkpts):
                ...
                temp_t2_v_12 = lib.einsum('inde,lmde->inlm',t2_1[ki,kn,kd].conj(), t2_1[kl,km,kd],optimize=True)
                M_ij[ki] += 0.5 * 1.25 * lib.einsum('inlm,jlnm->ij',temp_t2_v_12, eris_oooo[kj,kl,kn].conj(), optimize = True)
                M_ij[ki] -= 0.5 * 0.25 * lib.einsum('inlm,jmnl->ij',temp_t2_v_12, eris_oooo[kj,km,kn].conj(), optimize = True)
 
                M_ij[ki] += 0.5 * 1.25 * lib.einsum('jnlm,ilnm->ij',temp_t2_v_12.conj(), eris_oooo[ki,kl,kn], optimize = True)
                M_ij[ki] -= 0.5 * 0.25 * lib.einsum('jnlm,imnl->ij',temp_t2_v_12.conj(), eris_oooo[ki,km,kn], optimize = True)
                del temp_t2_v_12

                temp_t2_v_12_1 = lib.einsum('nide,mlde->inlm',t2_1[kn,ki,kd].conj(), t2_1[km,kl,kd],optimize=True)
                M_ij[ki] += 0.5 * 0.25 * lib.einsum('inlm,jlnm->ij',temp_t2_v_12_1, eris_oooo[kj,kl,kn].conj(), optimize = True)
                M_ij[ki] -= 0.5 * 0.25 * lib.einsum('inlm,jmnl->ij',temp_t2_v_12_1, eris_oooo[kj,km,kn].conj(), optimize = True)
                M_ij[ki] += 0.5 * 0.25 * lib.einsum('jnlm,ilnm->ij',temp_t2_v_12_1.conj(), eris_oooo[ki,kl,kn], optimize = True)
                M_ij[ki] -= 0.5 * 0.25 * lib.einsum('jnlm,imnl->ij',temp_t2_v_12_1.conj(), eris_oooo[ki,km,kn], optimize = True)

                temp_t2_v_13 = lib.einsum('inde,mlde->inml',t2_1[ki,kn,kd].conj(), t2_1[km,kl,kd],optimize=True)
                M_ij[ki] -= 0.5 * 0.5 * lib.einsum('inml,jlnm->ij',temp_t2_v_13, eris_oooo[kj,kl,kn].conj(), optimize = True)
                M_ij[ki] += 0.5 * 0.25 * lib.einsum('inml,jmnl->ij',temp_t2_v_13, eris_oooo[kj,km,kn].conj(), optimize = True)

                M_ij[ki] -= 0.5 * 0.5 * lib.einsum('jnml,ilnm->ij',temp_t2_v_13.conj(), eris_oooo[ki,kl,kn], optimize = True)
                M_ij[ki] += 0.5 * 0.25 * lib.einsum('jnml,imnl->ij',temp_t2_v_13.conj(), eris_oooo[ki,km,kn], optimize = True)
                del temp_t2_v_13

                temp_t2_v_13_1 = lib.einsum('nide,lmde->inml',t2_1[kn,ki,kd].conj(), t2_1[kl,km,kd],optimize=True)
                M_ij[ki] += 0.5 * 0.25 * lib.einsum('inml,jmnl->ij',temp_t2_v_13_1, eris_oooo[kj,km,kn].conj(), optimize = True)
                M_ij[ki] += 0.5 * 0.25 * lib.einsum('jnml,imnl->ij',temp_t2_v_13_1.conj(), eris_oooo[ki,km,kn], optimize = True)
                del temp_t2_v_13_1
            ...
            for kc in range(nkpts):
                ...
                if isinstance(eris.ovvv, type(None)):
                    ...
                    for p in range(0,nocc,chnk_size):
                        ...
                        k = eris_ovvv.shape[0]
                        s1 +=  2. * lib.einsum('icab,ibc->a', eris_ovvv.conj(), r2[ki,kb,a:a+k], optimize = True)
                        ...
                        s1 -=  lib.einsum('ibac,ibc->a',   eris_ovvv.conj(), r2[ki,kb,a:a+k], optimize = True)
                        del eris_ovvv
                        a += k
                else :
                    eris_ovvv = eris.ovvv[:]
                    s1 +=  2. * lib.einsum('icab,ibc->a', eris_ovvv[ki,kc,kshift].conj(), r2[ki,kb], optimize = True)
                    s2[ki,kb] += lib.einsum('icab,a->ibc', eris_ovvv[ki,kc,kshift], r1, optimize = True)
                    del eris_ovvv
                   ...
                   for ky in range(nkpts):
                       ...
                       for kz in range(nkpts):
                           ...
                           if isinstance(eris.vvvv, np.ndarray):
                               ...
                               s2[ki, kx] += np.dot(r2_1[ki,kw],eris_vvvv[kx,ky,kw].T.conj()).reshape(nocc,nvir,nvir)
                           elif isinstance(eris.vvvv, type(None)):
                       ...
                       for kj in range(nkpts):
                           ...
                           s2[ki,kx] -= 0.5*lib.einsum('iyzj,jzx->ixy',eris_ovvo[ki,ky,kz],r2[kj,kz],optimize = True)
                           s2[ki,kx] += lib.einsum('iyzj,jxz->ixy',eris_ovvo[ki,ky,kz],r2[kj,kx],optimize = True)
                           s2[ki,kx] -= 0.5*lib.einsum('ijzy,jxz->ixy',eris_oovv[ki,kj,kz],r2[kj,kx],optimize = True)

                           kz = kconserv[kj,ki,kx]
                           s2[ki,kx] -=  0.5*lib.einsum('ijzx,jzy->ixy',eris_oovv[ki,kj,kz],r2[kj,kz],optimize = True)

                           kw = kconserv[kj,ki,kx]
                           s2[ki,kx] -=  0.5*lib.einsum('ijwx,jwy->ixy',eris_oovv[ki,kj,kw],r2[kj,kw],optimize = True)

                           kw = kconserv[kj,ki,ky]
                           s2[ki,kx] -= 0.5*lib.einsum('ijwy,jxw->ixy',eris_oovv[ki,kj,kw],r2[kj,kx],optimize = True)
                           s2[ki,kx] += lib.einsum('iywj,jxw->ixy',eris_ovvo[ki,ky,kw],r2[kj,kx],optimize = True)
                           s2[ki,kx] -= 0.5*lib.einsum('iywj,jwx->ixy',eris_ovvo[ki,ky,kw],r2[kj,kw],optimize = True)
               ...
               for kx in range(nkpts):
                   for ky in range(nkpts):
                          ...
                          for kl in range(nkpts):
                              ...
                              temp = lib.einsum('b,lbmi->lmi',r1,eris_ovoo[kl,kb,km].conj(), optimize=True)
                              s2[ki,kx] += lib.einsum('lmi,lmxy->ixy',temp, t2_1[kl,km,kx].conj(), optimize=True)
                              del temp
               ...
               for kz in range(nkpts):
                   for kw in range(nkpts):
                          ...
                          for kl in range(nkpts):
                              ...
                              temp_s_a += lib.einsum('ljdw,jzw->lzd',t2_1[kl,kj,kd],r2[kj,kz],optimize=True)

                              temp_t2_r2_1 -= lib.einsum('ljwd,jzw->lzd',t2_1_ljw,r2[kj,kz],optimize=True)
                              del t2_1_ljw
                              temp_t2_r2_4 = lib.einsum('jldw,jwz->lzd',t2_1[kj,kl,kd],r2[kj,kw], optimize=True)
                              ...
                              if isinstance(eris.ovvv, type(None)):
                                  ...
                                  for p in range(0,nocc,chnk_size):
                                      ...
                                      s1 += 0.5*lib.einsum('lzd,ldza->a',temp_s_a[a:a+k],eris_ovvv,optimize=True)
                                      s1 += 0.5*lib.einsum('lzd,ldza->a',temp_t2_r2_1[a:a+k],eris_ovvv,optimize=True)
                                      del eris_ovvv
                                      
                                      eris_ovvv = dfadc.get_ovvv_df(adc, eris.Lov[kl,ka], eris.Lvv[kz,kd], p, chnk_size).reshape(-1,nvir,nvir,nvir)/nkpts
                                      s1 -= 0.5*lib.einsum('lzd,lazd->a',temp_s_a[a:a+k],eris_ovvv,optimize=True)
                                      s1 -= 0.5*lib.einsum('lzd,lazd->a',temp_t2_r2_4[a:a+k],eris_ovvv,optimize=True)
                                      del eris_ovvv
                                      a += k
                              else :
                                  ka = kconserv[kz, kd, kl]
                                  eris_ovvv = eris.ovvv[:]
                                  s1 += 0.5*lib.einsum('lzd,ldza->a',temp_s_a,eris_ovvv[kl,kd,kz],optimize=True)
                                  s1 += 0.5*lib.einsum('lzd,ldza->a',temp_t2_r2_1,eris_ovvv[kl,kd,kz],optimize=True)

                                  s1 -= 0.5*lib.einsum('lzd,lazd->a',temp_s_a,eris_ovvv[kl,ka,kz],optimize=True)
                                  s1 -= 0.5*lib.einsum('lzd,lazd->a',temp_t2_r2_4,eris_ovvv[kl,ka,kz],optimize=True)
                                  del eris_ovvv
                              del temp_s_a
                              del temp_t2_r2_1
                              del temp_t2_r2_4
               ...
               for kz in range(nkpts):
                   for kw in range(nkpts):
                          ...
                          for kl in range(nkpts):
                              ...
                              temp_s_a_1  =   -lib.einsum('jlzd,jwz->lwd',t2_1_jlz,r2[kj,kw],optimize=True)
                              temp_s_a_1 +=    lib.einsum('jlzd,jzw->lwd',t2_1_jlz,r2[kj,kz],optimize=True)
                              temp_t2_r2_2  = -lib.einsum('jlzd,jwz->lwd',t2_1_jlz,r2[kj,kw],optimize=True)
                              temp_t2_r2_2 +=  lib.einsum('jlzd,jzw->lwd',t2_1_jlz,r2[kj,kz],optimize=True)
                              temp_t2_r2_2 -=  lib.einsum('jlzd,jwz->lwd',t2_1_jlz,r2[kj,kw],optimize=True)
                              del t2_1_jlz
                              ...
                              temp_s_a_1 -= lib.einsum('ljdz,jwz->lwd',t2_1[kl,kj,kd],r2[kj,kw],optimize=True)
                              ...
                              if isinstance(eris.ovvv, type(None)):
                                  ...
                                  for p in range(0,nocc,chnk_size):
                                      ...
                                      k = eris_ovvv.shape[0]
                                      s1 -= 0.5*lib.einsum('lwd,ldwa->a',temp_s_a_1[a:a+k],eris_ovvv,optimize=True)
                                      s1 -= 0.5*lib.einsum('lwd,ldwa->a',temp_t2_r2_2[a:a+k],eris_ovvv,optimize=True)
                                      del eris_ovvv

                                      eris_ovvv = dfadc.get_ovvv_df(adc, eris.Lov[kl,ka], eris.Lvv[kw,kd], p, chnk_size).reshape(-1,nvir,nvir,nvir)/nkpts
                                      s1 += 0.5*lib.einsum('lwd,lawd->a',temp_s_a_1[a:a+k],eris_ovvv,optimize=True)
                                      s1 += 0.5*lib.einsum('lwd,lawd->a',temp_t2_r2_3[a:a+k],eris_ovvv,optimize=True)
                                      del eris_ovvv
                                      a += k
                                  ...
                                  ka = kconserv[kw, kd, kl]
                                  eris_ovvv = eris.ovvv[:]
                                  s1 -= 0.5*lib.einsum('lwd,ldwa->a',temp_s_a_1,eris_ovvv[kl,kd,kw],optimize=True)
                                  s1 += 0.5*lib.einsum('lwd,lawd->a',temp_s_a_1,eris_ovvv[kl,ka,kw],optimize=True)

                                  s1 -= 0.5*lib.einsum('lwd,ldwa->a',temp_t2_r2_2,eris_ovvv[kl,kd,kw],optimize=True)
                                  s1 += 0.5*lib.einsum('lwd,lawd->a',temp_t2_r2_3,eris_ovvv[kl,ka,kw],optimize=True)
                                  del eris_ovvv
               ...
               for kx in range(nkpts):
                   for ky in range(nkpts):
                          ...
                          for kl in range(nkpts):
                              ...
                              s2[ki,kx]  += lib.einsum('lxd,lidy->ixy',temp_1_1.conj(),t2_1[kl,ki,kd].conj(),optimize=True)
                              s2[ki,kx]  += lib.einsum('lxd,ilyd->ixy',temp_2_1.conj(),t2_1[ki,kl,ky].conj(),optimize=True)
                              s2[ki,kx]  -= lib.einsum('lxd,ildy->ixy',temp_2_1.conj(),t2_1[ki,kl,kd].conj(),optimize=True)

                              kd = kconserv[kl,kx,ki]
                              s2[ki,kx]  += lib.einsum('lyd,lixd->ixy',temp.conj(),t2_1[kl,ki,kx].conj(),optimize=True)
                              del temp
                              del temp_1_1
                              del temp_2_1
    ...
    if orb < nocc:
        ...
        for kk in range(nkpts):
            for kc in range(nkpts):
                ...
                T1 += 0.25*lib.einsum('kdc,ikdc->i',t2_1[kk,kshift,kd][:,orb,:,:].conj(), t2_1[ki,kk,kd], optimize = True)
                T1 -= 0.25*lib.einsum('kcd,ikdc->i',t2_1[kk,kshift,kc][:,orb,:,:].conj(), t2_1[ki,kk,kd], optimize = True)
                T1 -= 0.25*lib.einsum('kdc,ikcd->i',t2_1[kk,kshift,kd][:,orb,:,:].conj(), t2_1[ki,kk,kc], optimize = True)
                T1 += 0.25*lib.einsum('kcd,ikcd->i',t2_1[kk,kshift,kc][:,orb,:,:].conj(), t2_1[ki,kk,kc], optimize = True)
                T1 -= 0.25*lib.einsum('kdc,ikdc->i',t2_1[kshift,kk,kd][orb,:,:,:].conj(), t2_1[ki,kk,kd], optimize = True)
                T1 -= 0.25*lib.einsum('kcd,ikcd->i',t2_1[kshift,kk,kc][orb,:,:,:].conj(), t2_1[ki,kk,kc], optimize = True)
    else :
        if (adc.approx_trans_moments == False or adc.method == "adc(3)"):
            t1_2 = adc.t1[0]
        ...
        if orb < nocc:
            for kk in range(nkpts):
                for kc in range(nkpts):
                    ...
                    T1 += 0.25*lib.einsum('kdc,ikdc->i',t2_1[kk,ki,kd][:,orb,:,:].conj(), t2_2[ki,kk,kd], optimize = True)
                    T1 -= 0.25*lib.einsum('kcd,ikdc->i',t2_1[kk,ki,kc][:,orb,:,:].conj(), t2_2[ki,kk,kd], optimize = True)
                    T1 -= 0.25*lib.einsum('kdc,ikcd->i',t2_1[kk,ki,kd][:,orb,:,:].conj(), t2_2[ki,kk,kc], optimize = True)
                    T1 += 0.25*lib.einsum('kcd,ikcd->i',t2_1[kk,ki,kc][:,orb,:,:].conj(), t2_2[ki,kk,kc], optimize = True)
                    T1 -= 0.25*lib.einsum('kdc,ikdc->i',t2_1[ki,kk,kd][orb,:,:,:].conj(), t2_2[ki,kk,kd], optimize = True)
                    T1 -= 0.25*lib.einsum('kcd,ikcd->i',t2_1[ki,kk,kc][orb,:,:,:].conj(), t2_2[ki,kk,kc], optimize = True)

                    T1 += 0.25*lib.einsum('ikdc,kdc->i',t2_1[ki,kk,kd], t2_2[kk,ki,kd][:,orb,:,:].conj(),optimize = True)
                    T1 -= 0.25*lib.einsum('ikcd,kdc->i',t2_1[ki,kk,kc], t2_2[kk,ki,kd][:,orb,:,:].conj(),optimize = True)
                    T1 -= 0.25*lib.einsum('ikdc,kcd->i',t2_1[ki,kk,kd], t2_2[kk,ki,kc][:,orb,:,:].conj(),optimize = True)
                    T1 += 0.25*lib.einsum('ikcd,kcd->i',t2_1[ki,kk,kc], t2_2[kk,ki,kc][:,orb,:,:].conj(),optimize = True)
                    T1 -= 0.25*lib.einsum('ikcd,kcd->i',t2_1[ki,kk,kc], t2_2[ki,kk,kc][orb,:,:,:].conj(),optimize = True)
                    T1 -= 0.25*lib.einsum('ikdc,kdc->i',t2_1[ki,kk,kd], t2_2[ki,kk,kd][orb,:,:,:].conj(),optimize = True)
        else:
