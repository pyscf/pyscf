...
class KnownValues(unittest.TestCase):
    def test_n3_diffuse(self):
        ...
        self.assertAlmostEqual(e_mp2, -0.11464808675651168, 6)

        # IP-ADC(2)
        myadc.method = 'adc(2)'
        e, v, p, x = myadc.kernel(nroots=3,kptlist=[0])
        self.assertAlmostEqual(e[0][0],-1.1684386160049567,6)
        self.assertAlmostEqual(e[0][1],-1.0944210679002468,6)
        self.assertAlmostEqual(e[0][2],-1.0944210623845938,6)

        # Ensure kptlist behaves correctly
        e, v, p, x = myadc.kernel(nroots=3,kptlist=[1])
        self.assertAlmostEqual(e[0][0], -1.0362468821854942 , 6)
        self.assertAlmostEqual(e[0][1], -1.0015050525294895 , 6)
        self.assertAlmostEqual(e[0][2], -1.0015050525294888 , 6)

        # IP-ADC(3)
        myadc.method = 'adc(3)'
        e, v, p, x = myadc.kernel(nroots=3,kptlist=[0])
        self.assertAlmostEqual(e[0][0],-1.1740951190044178,6)
        self.assertAlmostEqual(e[0][1],-1.1536065878793587,6)
        self.assertAlmostEqual(e[0][2],-1.153606584783398,6)
