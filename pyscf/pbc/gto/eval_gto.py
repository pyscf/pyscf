...
def get_lattice_Ls(cell, nimgs=None, rcut=None, dimension=None, discard=True):
    ...
    grids_edge = lib.cartesian_prod([[-.5, .5]] * dimension).dot(a[:dimension])
...
if __name__ == '__main__':
    ...
    cell = gto.M(a=np.eye(3)*4, atom='He 1 1 1', basis=[[2,(1,.5),(.5,.5)]])
    coords = cell.get_uniform_grids([10]*3)
    ao_value = eval_gto(cell, "GTOval_sph", coords, kpts=cell.make_kpts([3]*3))
    print(lib.finger(np.asarray(ao_value)) - (-0.27594803231989179+0.0064644591759109114j))

    cell = gto.M(a=np.eye(3)*4, atom='He 1 1 1', basis=[[2,(1,.5),(.5,.5)]])
    coords = cell.get_uniform_grids([10]*3)
    ao_value = eval_gto(cell, "GTOval_ip_cart", coords, kpts=cell.make_kpts([3]*3))
    print(lib.finger(np.asarray(ao_value)) - (0.38051517609460028+0.062526488684770759j))
