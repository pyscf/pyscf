...
def intor_cross(intor, cell1, cell2, comp=None, hermi=0, kpts=None, kpt=None,
    ...
    kderiv = kwargs.get('kderiv', False)
    if kderiv:
        hermi = 0
        aosym = 's1'
        mat = np.empty((nkpts,3,ni,nj), dtype=np.complex128)
        for beta in range(3):
            out = np.empty((nkpts,1,ni,nj), dtype=np.complex128)
            igexpkL = expkL * Ls[:, beta] *1j
            drv(fintor, fill, out.ctypes.data_as(ctypes.c_void_p),
                ctypes.c_int(nkpts), ctypes.c_int(comp), ctypes.c_int(len(Ls)),
                Ls.ctypes.data_as(ctypes.c_void_p),
                igexpkL.ctypes.data_as(ctypes.c_void_p),
