...
def set_nuc_mod(mols, nucmod):
    for mol in mols:
        mol.build(nucmod=nucmod)

def reset_nuc_mod(mols):
    for mol in mols:
        mol.build(nucmod=None)
...
class KnownValues(unittest.TestCase):
    def test_sqrt_second_order(self):
        with lib.light_speed(10) as c:
            for nucmod in [None, 'G']:
                set_nuc_mod([mol, mol1, mol2], nucmod)
                nao = mol.nao_nr()
                aoslices = mol.aoslice_by_atom()
                p0, p1 = aoslices[0][2:]
    ...
    def test_hcore(self):
        with lib.light_speed(10) as c:
            ...
            mol.build(nucmod='G')
            h1_deriv_1_G = sfx2c1e_grad.gen_sf_hfw(mol, approx='1E')
            ha1_deriv_1_G = sfx2c1e_grad.gen_sf_hfw(mol, approx='ATOM1E')
            ...
            mol.build(nucmod='G')
            h1_deriv_2_G = sfx2c1e_grad.gen_sf_hfw(mol, approx='1E')
            ha1_deriv_2_G = sfx2c1e_grad.gen_sf_hfw(mol, approx='ATOM1E')
            ...
            h2_G = h2_deriv(0,0)
            h2_ref_G = (h1_deriv_1_G(0)[2] - h1_deriv_2_G(0)[2]) / 0.0002 * lib.param.BOHR
            self.assertAlmostEqual(abs(h2_G[2,2]-h2_ref_G).max(), 0, 6)
            self.assertAlmostEqual(lib.fp(h2_G), 33.71188114299618, 9)

            h2_G = h2_deriv(1,0)
            h2_ref_G = (h1_deriv_1_G(1)[2] - h1_deriv_2_G(1)[2]) / 0.0002 * lib.param.BOHR
            self.assertAlmostEqual(abs(h2_G[2,2]-h2_ref_G).max(), 0, 6)
            self.assertAlmostEqual(lib.fp(h2_G), -23.609411392574444, 7)

            h2_G = ha2_deriv(0,0)
            h2_ref_G = (ha1_deriv_1_G(0)[2] - ha1_deriv_2_G(0)[2]) / 0.0002 * lib.param.BOHR
            self.assertAlmostEqual(abs(h2_G[2,2]-h2_ref_G).max(), 0, 6)
            self.assertAlmostEqual(lib.fp(h2_G), 33.71866576690655, 9)


if __name__ == "__main__":
    print("Full Tests for sfx2c1e hessians")
    unittest.main()