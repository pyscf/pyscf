...
def add_mm_charges(scf_method, atoms_or_coords, charges, radii=None, unit=None,
    ...
    Kwargs:
        ...
        ecp : dict or str
            Input similar to to mol.ecp, ecplicit dict is prefered.
    ...
    mm_mol = mm_mole.create_mm_mol(atoms_or_coords, charges,
                                   radii=radii, unit=unit, ecp=ecp)
    if not numpy.allclose(charges, mm_mol.atom_charges()):
        idx_err = mm_mol.atom_charges() - numpy.array(charges) != 0
        for at, chg, fin_chg in zip(numpy.array(atoms_or_coords)[idx_err],
                                    numpy.array(charges)[idx_err],
                                    mm_mol.atom_charges()[idx_err]):
            logger.error(mol,
                f"Charge of mm_atom {at} should be {chg}, but is {fin_chg}")
        raise(RuntimeError("Inconsistent atoms_or_coords, charges, ecp"))
...
class QMMMSCF(QMMM):
    ...
    def get_hcore(self, mol=None):
        ...
        if mm_mol._ecp:
            ecp_idx = [i for i in range(mm_mol.natm)
                       if mm_mol.atom_symbol(i) in mm_mol._ecp.keys()]
            ecp_atoms = [mm_mol._atom[i] for i in ecp_idx]
            fakemol = gto.Mole(verbose=0)
            fakemol.build(
                atom=mol._atom + ecp_atoms, basis=mol._basis, ecp=mm_mol._ecp,
                unit='Bohr', verbose=0, nucmod=mol.nucmod, dump_input=False,
                output=mol.output, pseudo=None, charge=mol.charge,
                spin=None, symmetry=mol.symmetry,
                symmetry_subgroup=mol.symmetry_subgroup)
            h1e += fakemol.intor_symmetric('ECPscalar')
        return h1e
...
def add_mm_charges_grad(scf_grad, atoms_or_coords, charges, radii=None,
                        unit=None, ecp=None):
    ...
    if ecp is not None:
        raise(NotImplementedError("Embedding ECPs for QM gradients"))
    mol = scf_grad.mol
    if unit is None:
        unit = mol.unit
    mm_mol = mm_mole.create_mm_mol(atoms_or_coords, charges,
