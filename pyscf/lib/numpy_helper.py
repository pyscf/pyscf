...
def sum(a, axis=None, dtype=None, out=None, **kwargs):
    '''`numpy.sum` with multithreading
    '''
    if getattr(a, 'ndim', 1) == 1 or axis is None or kwargs:
        return numpy.sum(a, axis, dtype, out, **kwargs)

    if dtype is None:
        dtype = a.dtype
    if dtype == numpy.double:
        fn = getattr(_np_helper, "NPdsum")
    elif dtype == numpy.complex128:
        fn = getattr(_np_helper, "NPzsum")
    else:
        return numpy.sum(a, axis, dtype, out, **kwargs)
    ...
    if axis == -1:
        ...
        if out is None:
            ...
            if out.shape == out_shape:
                raise ValueError(f'out.shape={out.shape} != out_shape')
            if ((not out.flags.c_contiguous) or out.dtype != dtype):
                return numpy.sum(a, axis, dtype, out, **kwargs)
        ...
        if out is None:
            ...
            if out.shape == out_shape:
                raise ValueError(f'out.shape={out.shape} != out_shape')
            if ((not out.flags.f_contiguous) or out.dtype != dtype):
                return numpy.sum(a, axis, dtype, out, **kwargs)
        a = numpy.asarray(a, order='F', dtype=dtype)
        lda = a.shape[0]

    fn(out.ctypes.data_as(ctypes.c_void_p),
       a.ctypes.data_as(ctypes.c_void_p),
       ctypes.c_int(lda), ctypes.c_int(out.size))
    return out

def add(a, b, out=None, **kwargs):
    '''`numpy.add` with multithreading
    '''
...
def subtract(a, b, out=None, **kwargs):
    '''`numpy.subtract` with multithreading
    '''
    if (a.shape != b.shape or a.dtype != b.dtype or
        not(a.flags.c_contiguous or a.flags.f_contiguous) or
        not(b.flags.c_contiguous or b.flags.f_contiguous)):
...
def multiply(a, b, out=None, **kwargs):
    '''`numpy.multiply` with multithreading
    '''
...
def multiply_sum(a, b, axis=0):
    '''Composition of multiply and sum.
    '''
    dtype = numpy.result_type(a, b)
    order = 'C'
    if axis == 0:
        order = 'F'
    a = numpy.asarray(a, order=order, dtype=dtype)
    b = numpy.asarray(b, order=order, dtype=dtype)
    return sum(multiply(a, b), axis=axis)

def reciprocal(x, out=None, order='K', dtype=None, **kwargs):
    '''`numpy.reciprocal` with multithreading
    '''
...
def concatenate(arrays, axis=0, out=None, dtype=None, casting="same_kind"):
    '''`numpy.concatenate` with multithreading
    '''
    use_numpy = False
...
def copy(a, order='K', subok=False, dtype=None, out=None):
    '''`numpy.copy` with multithreading, and allows type change.
    '''
...
def vdot(a, b):
    '''`numpy.vdot` with multithreading
    '''
