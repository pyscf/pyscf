...
static void apply_Dmats_new(double* out, double* ints, double *Dmats, int *rot_loc, int rot_mat_size, int iop,
                           int di, int dj, int dk, int l_i, int l_j, int l_k,   
                           int nci, int ncj, int nck, double* tmp, void* gemm_i, void* gemm_j, void* gemm_k)
{                                                                               
    double *pDmats, *pTk, *pTj, *pTi;                                           
    pDmats = Dmats+(size_t)rot_mat_size*iop;                                    
    pTk = pDmats + rot_loc[l_k];                                                
    ...
    //const double D1 = 1;
    int k;
    //double *tmp = malloc(sizeof(double) * dim_Tjk*dim_Ti);
    //dgemm_(&TRANS_N, &TRANS_N, &dim_Ti, &dim_Tjk, &dim_Ti,
    //       &D1, Ti, &dim_Ti, ints, &dim_Ti,
    //       &D0, tmp, &dim_Ti);


    dgemm_jit_kernel_t dgemm_i = mkl_jit_get_dgemm_ptr(gemm_i);
    dgemm_jit_kernel_t dgemm_j = mkl_jit_get_dgemm_ptr(gemm_j);
    dgemm_jit_kernel_t dgemm_k = mkl_jit_get_dgemm_ptr(gemm_k);

    dgemm_i(gemm_i, Ti, ints, tmp);
    for (k=0; k<dim_Tk; k++) {
        double *ptmp = tmp + k*dim_Tj*dim_Ti;
        //dgemm_(&TRANS_N, &TRANS_T, &dim_Ti, &dim_Tj, &dim_Tj,                   
        //       &D1, ptmp, &dim_Ti, Tj, &dim_Tj,                                 
        //       &D0, ptmp, &dim_Ti); 
        dgemm_j(gemm_j, ptmp, Tj, ptmp);
    }

    //int dim_TiTj = dim_Ti * dim_Tj;
    //dgemm_(&TRANS_N, &TRANS_T, &dim_TiTj, &dim_Tk, &dim_Tk,                   
    //       &D1, tmp, &dim_TiTj, Tk, &dim_Tk,                                  
    //       &D0, out, &dim_TiTj);   
    dgemm_k(gemm_k, tmp, Tk, out);                                                                        
                          ...
                          int *L2iL, int *ops, double *Dmats, int *rot_loc, int nop, int rot_mat_size, double* t_cpu, double* tD)
{
    ...
    for (ksh = ksh0; ksh < ksh1; ksh++) {//loop over shells one by one
        ...
        shls[2] = ksh;
        int *int_flags_L2 = malloc(sizeof(int)*nimgs*nimgs);
        ...
        double *tmp = malloc(sizeof(double) * dijmc);
        void *gemm_i, *gemm_j, *gemm_k;
        mkl_jit_status_t status_i = mkl_jit_create_dgemm(&gemm_i, CblasColMajor, CblasNoTrans, CblasNoTrans, di, dkj, di, 1.0, di, di, 0.0, di);        
        mkl_jit_status_t status_j = mkl_jit_create_dgemm(&gemm_j, CblasColMajor, CblasNoTrans, CblasTrans, di, dj, dj, 1.0, di, dj, 0.0, di);
        mkl_jit_status_t status_k = mkl_jit_create_dgemm(&gemm_k, CblasColMajor, CblasNoTrans, CblasTrans, di*dj, dkmax, dkmax, 1.0, di*dj, dkmax, 0.0, di*dj);
        assert(status_i != MKL_JIT_ERROR && status_j != MKL_JIT_ERROR && status_k != MKL_JIT_ERROR);
        ...
        for (iL0 = 0; iL0 < nimgs; iL0+=IMGBLK) {
            ...
            for (iL = iL0; iL < iL0+iLcount; iL++) {
                ...
                */
                    ...
                    iop = piop[idx];
                    pint_ijk = int_ijk_buf+(size_t)dijmc*idx_L2;
                    if (int_flags_L2[idx_L2] == 0){//build integral
                        clock_t CPU_time_1 = clock();
                        int_flags_L2[idx_L2] = -1;
                        int iL_irr = idx_L2 / nimgs;
                        int jL_irr = idx_L2 % nimgs;
                        shift_bas(env_loc, env, Ls, iptrxyz, iL_irr);
                        shift_bas(env_loc, env, Ls, jptrxyz, jL_irr);
                    ...
                    clock_t CPU_time_1 = clock();
                    if (int_flags_L2[idx_L2] == -1) {
                        for (i = 0; i < dijmc; i++) {
                            pbuf[i] = 0;
                        }
                        //memset(pbuf, 0, dijmc*sizeof(double));
                    }
                    else if (idx_L2 == idx) {
                        //clock_t CPU_time_1 = clock();
                        memcpy(pbuf, pint_ijk, (size_t)dijmc*sizeof(double));
                        //clock_t CPU_time_2 = clock();
                        //*tD += (double)(CPU_time_2-CPU_time_1)/CLOCKS_PER_SEC;
                    }
                    else {
                        //clock_t CPU_time_1 = clock();
                        apply_Dmats_new(pbuf, pint_ijk, Dmats, rot_loc, rot_mat_size, iop,
                                    mi, mj, mk, l_i, l_j, l_k, nci, ncj, nck, tmp, 
        ...
        */
        ...
        mkl_jit_destroy(gemm_i);
        mkl_jit_destroy(gemm_j);
        mkl_jit_destroy(gemm_k);
    }
}
