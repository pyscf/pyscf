                                     ...
                                     int *shls_lim, int *ao_loc)
{
        ...
        int lim1 = shls_lim[1];
        if (lim0 >= lim1) {
                return CVHFshls_block_partition(block_loc, shls_slice, ao_loc);
        }

        int ish0 = shls_slice[0];
        int ish1 = shls_slice[1];
        int seg[4] = {ish0, lim0, lim1, ish1};
        int count = CVHFshls_block_partition(block_loc, seg, ao_loc);
        count += CVHFshls_block_partition(block_loc+count, seg+1, ao_loc);
        count += CVHFshls_block_partition(block_loc+count, seg+2, ao_loc);
        return count;
}
                       ...
                       int *atm, int natm, int *bas, int nbas, double *env)
{
        ...
        size_t nblock_i = CVHFshls_block_partition(block_iloc, shls_slice+0, ao_loc);
        size_t nblock_j = CVHFshls_block_partition(block_jloc, shls_slice+2, ao_loc);
        size_t nblock_k = CVHFshls_block_partition(block_kloc, shls_slice+4, ao_loc);
        size_t nblock_l = CVHFshls_block_partition(block_lloc, shls_slice+6, ao_loc);
