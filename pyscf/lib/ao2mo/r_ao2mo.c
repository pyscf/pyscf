                    ...
                    struct _AO2MOEnvs *envs, int seekdim)
{
        ...
        {
        case 1:
                return envs->bra_count * envs->ket_count;
        case 2:
                return envs->nao * envs->nao;
        }
        const double D0 = 0;
        const double D1 = 1;
                    ...
                    int nkl, int ish, int jshtot, struct _AO2MOEnvs *envs)
{
        ...
        int icomp, i, j, k, l, n;
        int shls[4];
        double complex *buf = malloc(sizeof(double complex) * di * nao * NCTRMAX * NCTRMAX * envs->ncomp);
        assert(buf);
        ...
        for (kl = klsh_start; kl < klsh_end; kl++)
        {
                ...
                for (jsh = 0; jsh < jshtot; jsh++)
                {
                        ...
                        {
                                peri = eri + nao2 * nkl * icomp + ao_loc[ish] * nao + ao_loc[jsh];
                                for (k = 0; k < dk; k++)
                                {
                                        for (l = 0; l < dl; l++)
                                        {
                    ...
                    int nkl, int ish, int jshtot, struct _AO2MOEnvs *envs)
{
        ...
        int icomp, i, j, k, l, n;
        int shls[4];
        double complex *buf = malloc(sizeof(double complex) * di * nao * nkl * envs->ncomp);
        assert(buf);
        double complex *pbuf, *pbuf1, *peri;
                   ...
                   int *atm, int natm, int *bas, int nbas, double *env)
{
        ...
        double complex *eri_ao = malloc(sizeof(double complex) * nao * nao * nkl * ncomp);
        assert(eri_ao);
        int ish, kl;
        int (*fprescreen)();
        if (vhfopt != NULL)
...
#pragma omp parallel default(none) \
    shared(fill, fprescreen, eri_ao, envs, intor, nkl, nbas) private(ish)
...
#pragma omp parallel default(none) \
    shared(ftrans, fmmm, eri, eri_ao, nkl, ncomp, envs) private(kl)
...
#pragma omp parallel default(none) \
    shared(ftrans, fmmm, vout, vin, nijcount, envs) private(i)
#pragma omp for nowait schedule(static)
