...
def gamma2_intermediates(cc, t1, t2, l1, l2):
    ...
    tauab = t2ab + numpy.einsum('ia,jb->ijab', 2*t1a, t1b)
    ...
    goOvV = (l2ab.conj()*2 + tauab+t2ab) * .25
    ...
    goOvV -= einsum('ikab,kj->ijab', tauab+t2ab , tmpb) * .25
    goOvV -= einsum('kiba,kj->jiba', tauab+t2ab , tmpa) * .25
    ...
    goOvV -= einsum('lj,ilab->ijab', tmpb, tauab+t2ab) * .125
    goOvV -= einsum('lj,liba->jiba', tmpa, tauab+t2ab) * .125
    ...
    goOvV -= einsum('db,ijad->ijab', tmpb, tauab+t2ab) * .125
    goOvV -= einsum('db,jida->jiba', tmpa, tauab+t2ab) * .125
    ...
    goOvV += einsum('lDiA,lJbD->iJbA', miAjB, tauab) * .25
    goOvV += einsum('LdIa,jLdB->jIaB', mIaJb, tauab) * .25

    tauab += t2ab
    tmpaa = einsum('klcd,ijcd->ijkl', l2aa, tauaa) * .25**2
    tmpbb = einsum('klcd,ijcd->ijkl', l2bb, taubb) * .25**2
    tmpabab = einsum('kLcD,iJcD->iJkL', l2ab, tauab) * .25**2
    goovv += einsum('ijkl,klab->ijab', tmpaa, tauaa)
    goOvV += einsum('ijkl,klab->ijab', tmpabab, tauab) * 2
    ...
    govvv = einsum('ja,ijcb->iacb', l1a, tauaa) * .25
    goVvV = einsum('ja,ijcb->iacb', l1b, tauab) * .25
    gOvVv = einsum('ja,jibc->iacb', l1a, tauab) * .25
    gOVVV = einsum('ja,ijcb->iacb', l1b, taubb) * .25
...
def _make_rdm2(mycc, t1, t2, l1, l2, d2=None):
    ...
    d2aa, d2ab, d2ba, d2bb = d2
    dovov, dvvvv, doooo, doovv, dovvo, dvvov, dovvv, dooov = d2aa
    dovOV, dvvVV, dooOO, dooVV, dovVO, dvvOV, dovVV, dooOV = d2ab
    dOVov, dVVvv, dOOoo, dOOvv, dOVvo, dVVov, dOVvv, dOOov = d2ba
    dOVOV, dVVVV, dOOOO, dOOVV, dOVVO, dVVOV, dOVVV, dOOOV = d2bb
