...
def update_lambda(mycc, t1, t2, l1, l2, eris, imds):
    ...
    fov1 = fova + numpy.einsum('kcjb,kc->jb', ovov1, t1a)
    fov1+= numpy.einsum('jbKC,KC->jb', ovOV1, t1b)
    ...
    fov1 = fovb + numpy.einsum('kcjb,kc->jb', OVOV1, t1b)
    fov1+= numpy.einsum('kcJB,kc->JB', ovOV1, t1a)
    ...
    tmp  = fova - numpy.einsum('kbja,jb->ka', ovov1, t1a)
    tmp += numpy.einsum('kaJB,JB->ka', ovOV1, t1b)
    u1a -= numpy.einsum('ik,ka->ia', moo, tmp)
    u1a -= numpy.einsum('ca,ic->ia', mvv, tmp)
    tmp  = fovb - numpy.einsum('kbja,jb->ka', OVOV1, t1b)
    tmp += numpy.einsum('jbKA,jb->KA', ovOV1, t1a)
