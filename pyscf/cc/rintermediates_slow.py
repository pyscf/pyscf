...
def cc_Woooo(t1, t2, eris):
    Wklij = np.asarray(eris.oooo).copy()
...
def cc_Wvvvv(t1, t2, eris):
    # Incore 
    Wabcd = np.asarray(eris.vvvv).copy()
    Wabcd -= einsum('akcd,kb->abcd', eris.vovv, t1)
    Wabcd -= einsum('bkdc,ka->abcd', eris.vovv, t1)
    return Wabcd

#X    ## HDF5
#X    if t1.dtype == np.complex: ds_type = 'c16'
#X    else: ds_type = 'f8'
#X    _tmpfile1 = tempfile.NamedTemporaryFile(dir=lib.param.TMPDIR)
#X    fimd = h5py.File(_tmpfile1.name)
#X    nocc,nvir = t1.shape
#X    Wabcd = fimd.create_dataset('vvvv', (nvir,nvir,nvir,nvir), ds_type)
#X    # avoid transpose inside loop
#X    ovvv = np.asarray(eris.vovv).transpose(1,0,3,2).copy()
#X    for a in range(nvir):
#X        Wabcd[a] = eris.vvvv[a]
#X        Wabcd[a] += -einsum('kcd,kb->bcd', eris.vovv[a], t1)
#X        #Wabcd[a] += -einsum('bkdc,k->bcd', eris.vovv, t1[:,a])
#X        Wabcd[a] += -einsum('k,kbcd->bcd', t1[:,a],ovvv)
#X    return Wabcd

def cc_Wvoov(t1, t2, eris):
    Wakic = np.asarray(eris.voov).copy()
...
def cc_Wvovo(t1, t2, eris):
    Wakci = np.asarray(eris.ovov).transpose(1,0,3,2).copy()
...
def W1ovvo(t1, t2, eris):
    Wkaci = np.asarray(eris.voov).transpose(1,0,3,2).copy()
...
def W1ovov(t1, t2, eris):
    Wkbid = np.asarray(eris.ovov).copy()
...
def Wvvvo(t1, t2, eris,_Wvvvv=None):
    nocc,nvir = t1.shape
    Wabcj = np.asarray(eris.vovv).transpose(2,3,0,1).conj().copy()
...
def Wovoo(t1, t2, eris):
    Wkbij = np.asarray(eris.ooov).transpose(2,3,0,1).conj().copy()
