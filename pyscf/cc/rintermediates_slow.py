...
def cc_Wvoov(t1, t2, eris):
    Wakic = np.asarray(eris.ovvo).transpose(2,0,3,1).copy()
    eris_ovvv = np.asarray(eris.ovvv)
...
def cc_Wvovo(t1, t2, eris):
    Wakci = np.asarray(eris.oovv).transpose(2,0,3,1).copy()
    eris_ovvv = np.asarray(eris.ovvv)
...
def Wvovv(t1, t2, eris):
    eris_ovov = eris.ovov
    Walcd = np.asarray(eris.ovvv)
    Walcd -= einsum('ka,kcld->alcd', t1, eris_ovov)
    return Walcd

def W1ovvo(t1, t2, eris):
    Wkaci = np.asarray(eris.ovvo).transpose(0,2,1,3).copy()
    eris_ovov = eris.ovov
...
def W1ovov(t1, t2, eris):
    Wkbid = np.asarray(eris.oovv).transpose(0,2,1,3).copy()
    eris_ovov = eris.ovov
