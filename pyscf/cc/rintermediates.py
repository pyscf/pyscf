...
def Loo(t1, t2, eris):
    ...
    Lki += 2*einsum('kilc,lc->ki', eris_ooov, t1)
    Lki -=   einsum('likc,lc->ki', eris_ooov, t1)
    return Lki
...
def cc_Woooo(t1, t2, eris):
    ...
    eris_ooov = np.asarray(eris.vooo).conj().transpose(3,2,1,0)
    Wklij += einsum('kilc,jc->klij', eris_ooov, t1)
    Wklij += einsum('ljkc,ic->klij', eris_ooov, t1)
    eris_ovov = eris.vovo.conj().transpose(1,0,3,2)
...
def cc_Wvoov(t1, t2, eris):
    ...
    eris_ovvv = np.asarray(eris.vovv).conj().transpose(1,0,3,2)
    eris_ooov = np.asarray(eris.vooo).conj().transpose(3,2,1,0)
    Wakic -= einsum('likc,la->akic', eris_ooov, t1)
...
def cc_Wvovo(t1, t2, eris):
    ...
    eris_ovvv = np.asarray(eris.vovv).conj().transpose(1,0,3,2)
    eris_ooov = np.asarray(eris.vooo).conj().transpose(3,2,1,0)
    Wakci -= einsum('kilc,la->akci', eris_ooov, t1)
...
def Woooo(t1, t2, eris):
    ...
    Wklij += einsum('kcld,ic,jd->klij', eris_ovov, t1, t1)
    eris_ooov = np.asarray(eris.vooo).conj().transpose(3,2,1,0)
    Wklij += einsum('kild,jd->klij', eris_ooov, t1)
    Wklij += einsum('ljkc,ic->klij', eris_ooov, t1)
    return Wklij
...
def Wvvvo(t1, t2, eris, _Wvvvv=None):
    ...
    eris_ooov = np.asarray(eris.vooo).conj().transpose(3,2,1,0)
    Wabcj +=   einsum('ljkc,lkba->abcj', eris_ooov, t2)
    Wabcj +=   einsum('ljkc,lb,ka->abcj', eris_ooov, t1, t1)
...
def Wovoo(t1, t2, eris):
    ...
    Wkbij += 2*einsum('kild,ljdb->kbij', eris_ooov, t2)
    Wkbij +=  -einsum('kild,jldb->kbij', eris_ooov, t2)
    Wkbij +=  -einsum('likd,ljdb->kbij', eris_ooov, t2)
    Wkbij +=   einsum('kcbd,jidc->kbij', eris_ovvv, t2)
    Wkbij +=   einsum('kcbd,jd,ic->kbij', eris_ovvv, t1, t1)
    Wkbij +=  -einsum('ljkc,libc->kbij', eris_ooov, t2)
