...
parser.add_argument("--ircs", type=float, nargs=3, default=[0.7, 2.0, 0.1])
...
for ircidx, irc in enumerate(ircs):
    ...
    if args.full_ccsd:
        ...
        if MPI_rank == 0:
            ...
            with open(args.output, "a") as f:
                f.write("%3f  %.8e  %.8e  %.8e  %.8e  %.8e  %.8e\n" % (irc, mf.e_tot, cc.e_tot, mf.e_tot+cc.e_ccsd_v, mf.e_tot+oh_cluster.e_ccsd, mf.e_tot+oh_cluster.e_ccsd_v, mf.e_tot+oh_cluster.e_cl_ccsd))
