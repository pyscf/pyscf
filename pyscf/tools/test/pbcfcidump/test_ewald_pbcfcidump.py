...
cell = build_cell(ase_atom, ke=ke_cutoff, basis=basis, incore_anyway=True,
                 pseudo=None)
mf, scaled_kpts = run_khf(cell, nmp=nmp, gamma=True, exxdiv='ewald')
...
def compare_intdumps(filename_new, filename_bench, exchange):
    ...
    print("Difference in magnitude of integrals greater than 1e-4:")
    print(df_m.query('abs_mag_diff > 1e-4'))
    print("Difference in real value of integrals greater than 1e-4:")
    print(df_m.query('abs_real_diff > 1e-4'))
    print("Difference in imag value of integrals greater than 1e-4:")
    print(df_m.query('abs_imag_diff > 1e-4'))
    return df_m
...
class CalcIntegrals(unittest.TestCase):
    ...
    def test_setup(self):
        # SCF energy
        self.assertAlmostEqual(mf.e_tot, -0.748480339611342)
        # scaled_kpts
    ...
    def test_overall_fcidump(self):
        ...
        df_m  = compare_intdumps(fcid, fcid+"_e", False)
        self.assertTrue(df_m.query('abs_mag_diff > 1e-4').empty)
        self.assertTrue(df_m.query('abs_real_diff > 1e-4').empty)
        self.assertTrue(df_m.query('abs_imag_diff > 1e-4').empty)
        print("testing fcidump_x")
        df_m = compare_intdumps(fcid+"_X", fcid+"_e_X", True)
        self.assertTrue(df_m.query('abs_mag_diff > 1e-4').empty)
        self.assertTrue(df_m.query('abs_real_diff > 1e-4').empty)
        self.assertTrue(df_m.query('abs_imag_diff > 1e-4').empty)
