...
class KnownValues(unittest.TestCase):
    def test_from_chkfile(self):
        with tempfile.NamedTemporaryFile(dir=lib.param.TMPDIR) as tmpfcidump:
            wfn_format.from_chkfile(tmpfcidump.name, mf.chkfile)

    def test_from_scf(self):
        with tempfile.NamedTemporaryFile(dir=lib.param.TMPDIR) as tmpfcidump:
            wfn_format.from_scf(mf, tmpfcidump.name)

    def test_from_mo(self):
        with tempfile.NamedTemporaryFile(dir=lib.param.TMPDIR) as tmpfcidump:
            wfn_format.from_mo(mf.mol, tmpfcidump.name, mf.mo_coeff)


if __name__ == "__main__":
    print("Full Tests for wfn_format")
    unittest.main()
