...
dmrgsolver1 = dmrgci.DMRGCI(mol)
dmrgsolver1.nroots = 2
dmrgsolver1.weights = [.5, .5]
dmrgsolver1.scratchDirectory = '/scratch/dmrg1'
dmrgsolver2 = dmrgci.DMRGCI(mol)
dmrgsolver2.scratchDirectory = '/scratch/dmrg2'

class FakeCISolver(dmrgci.DMRGCI):
    def kernel(self, h1, h2, ncas, nelecas, ci0=None, **kwargs):
        # singlet
        e1, r1 = dmrgsolver1.kernel(h1, h2, ncas, (nelecas//2,nelecas//2))
        # triplet
        e2, r2 = dmrgsolver2.kernel(h1, h2, ncas, (nelecas//2+1,nelecas//2-1))
    ...
    def approx_kernel(self, h1, h2, norb, nelec, ci0=None, **kwargs):
        # singlet
        e1, r1 = dmrgsolver1.approx_kernel(h1, h2, ncas, (nelecas//2,nelecas//2))
        # triplet
        e2, r2 = dmrgsolver2.approx_kernel(h1, h2, ncas, (nelecas//2+1,nelecas//2-1))
        e_avg = e1[0]*(weights[0]+weights[1]) + e2*weights[2]
        return e_avg, [r1[0], r1[1], r2]

    def make_rdm1(self, ci0, norb, nelec):
        dm1_1 = dmrgsolver1.make_rdm1(0, norb, (nelec//2,nelec//2))
        dm1_2 = dmrgsolver1.make_rdm1(1, norb, (nelec//2,nelec//2))
        dm1_3 = dmrgsolver2.make_rdm1(0, norb, (nelec//2+1,nelec//2-1))
        rdm1 = dm1_1 * weights[0] + dm1_2 * weights[1] + dm1_3 * weights[2]
        return rdm1

    def make_rdm12(self, state, norb, nelec):
        dm12_1 = dmrgsolver1.make_rdm12(0, norb, (nelec//2,nelec//2))
        dm12_2 = dmrgsolver1.make_rdm12(1, norb, (nelec//2,nelec//2))
        dm12_3 = dmrgsolver2.make_rdm12(0, norb, (nelec//2+1,nelec//2-1))
...
dmrgsolver2.scratchDirectory = '/scratch/dmrg2'
dmrgsolver3 = dmrgci.DMRGCI(mol)
dmrgsolver3.wfnsym = 'B1u'
dmrgsolver3.scratchDirectory = '/scratch/dmrg3'

class FakeCISolver(dmrgci.DMRGCI):
    def kernel(self, h1, h2, ncas, nelecas, ci0=None, **kwargs):
        e1, r1 = dmrgsolver1.kernel(h1, h2, ncas, nelecas)
        e2, r2 = dmrgsolver2.kernel(h1, h2, ncas, nelecas)
        e3, r3 = dmrgsolver3.kernel(h1, h2, ncas, nelecas)
        e_avg = e1*weights[0] + e2*weights[1] + e3*weights[2]
        return e_avg, [r1, r2, r3]

    def approx_kernel(self, h1, h2, norb, nelec, ci0=None, **kwargs):
        e1, r1 = dmrgsolver1.approx_kernel(h1, h2, ncas, nelecas)
        e2, r2 = dmrgsolver2.approx_kernel(h1, h2, ncas, nelecas)
        e3, r3 = dmrgsolver3.approx_kernel(h1, h2, ncas, nelecas)
        e_avg = e1*weights[0] + e2*weights[1] + e3*weights[2]
        return e_avg, [r1, r2, r3]

    def make_rdm1(self, ci0, norb, nelec):
        dm1_1 = dmrgsolver1.make_rdm1(0, norb, nelec)
        dm1_2 = dmrgsolver2.make_rdm1(0, norb, nelec)
        dm1_3 = dmrgsolver3.make_rdm1(0, norb, nelec)
        rdm1 = dm1_1 * weights[0] + dm1_2 * weights[1] + dm1_3 * weights[2]
        return rdm1

    def make_rdm12(self, state, norb, nelec):
        dm12_1 = dmrgsolver1.make_rdm12(0, norb, nelec)
        dm12_2 = dmrgsolver2.make_rdm12(0, norb, nelec)
        dm12_3 = dmrgsolver3.make_rdm12(0, norb, nelec)
        rdm1 = dm12_1[0] * weights[0] + dm12_2[0] * weights[1] + dm12_3[0] * weights[2]
        rdm2 = dm12_1[1] * weights[0] + dm12_2[1] * weights[1] + dm12_3[1] * weights[2]
        return rdm1, rdm2
