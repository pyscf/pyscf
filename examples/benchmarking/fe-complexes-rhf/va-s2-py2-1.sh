...
#PBS -l mem=220200960000b
#PBS -l vmem=220200960000b
#PBS -M iu254@stud.uni-heidelberg.de
#PBS -m ae
#PBS -l nodes=1:ppn=8
#
###################################
...
RETURN_VALUE=0
NODE_WORKDIR="/scratch/ccprak16/va-s2-py2-1_$PBS_JOBID"
NODE_SCRATCHDIR="/lscratch/ccprak16/va-s2-py2-1_$PBS_JOBID"
#
###################################
#
 print_info() {
...
payload_hooks() {
:
for FILEORDIRPATH in $PBS_O_WORKDIR/va-s2-py2-1.in; do
...
/export/home/ccprak16/bin/versions/q-chem/qchem-5.2 -nt 8 "va-s2-py2-1.in" "va-s2-py2-1.out"
RETURN_VALUE=$?

# check if job terminated successfully
if ! tail -n 30 "va-s2-py2-1.out" | grep -q "Thank you very much for using Q-Chem.  Have a nice day."; then
    RETURN_VALUE=1
fi

for FILEORDIRPATH in $NODE_WORKDIR/va-s2-py2-1.out; do
...
for FILEORDIRPATH in $NODE_WORKDIR/va-s2-py2-1.in.fchk; do
    FILEORDIR=$(echo "$FILEORDIRPATH" | sed -e s@^$NODE_WORKDIR//*@@)
    if [ -r "$FILEORDIRPATH" ]; then 
        CPARGS="--dereference" 
        [ -d "$FILEORDIRPATH" ] && CPARGS="--recursive"
...
for FILEORDIRPATH in $NODE_WORKDIR/va-s2-py2-1.out.plots; do
    FILEORDIR=$(echo "$FILEORDIRPATH" | sed -e s@^$NODE_WORKDIR//*@@)
    if [ -r "$FILEORDIRPATH" ]; then 
        CPARGS="--dereference" 
        [ -d "$FILEORDIRPATH" ] && CPARGS="--recursive"
