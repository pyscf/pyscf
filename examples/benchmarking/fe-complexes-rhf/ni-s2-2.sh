...
RETURN_VALUE=0
NODE_WORKDIR="/scratch/ccprak16/ni-s2-2_$PBS_JOBID"
NODE_SCRATCHDIR="/lscratch/ccprak16/ni-s2-2_$PBS_JOBID"
#
###################################
#
 print_info() {
...
payload_hooks() {
:
for FILEORDIRPATH in $PBS_O_WORKDIR/ni-s2-2.in; do
...
/export/home/ccprak16/bin/versions/q-chem/qchem-5.2 -nt 8 "ni-s2-2.in" "ni-s2-2.out"
RETURN_VALUE=$?

# check if job terminated successfully
if ! tail -n 30 "ni-s2-2.out" | grep -q "Thank you very much for using Q-Chem.  Have a nice day."; then
    RETURN_VALUE=1
fi

for FILEORDIRPATH in $NODE_WORKDIR/ni-s2-2.out; do
...
for FILEORDIRPATH in $NODE_WORKDIR/ni-s2-2.in.fchk; do
    FILEORDIR=$(echo "$FILEORDIRPATH" | sed -e s@^$NODE_WORKDIR//*@@)
    if [ -r "$FILEORDIRPATH" ]; then 
        CPARGS="--dereference" 
        [ -d "$FILEORDIRPATH" ] && CPARGS="--recursive"
...
for FILEORDIRPATH in $NODE_WORKDIR/ni-s2-2.out.plots; do
    FILEORDIR=$(echo "$FILEORDIRPATH" | sed -e s@^$NODE_WORKDIR//*@@)
    if [ -r "$FILEORDIRPATH" ]; then 
        CPARGS="--dereference" 
        [ -d "$FILEORDIRPATH" ] && CPARGS="--recursive"
