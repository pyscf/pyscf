...
'''
Examples of RCCSDT calculations.

This script demonstrates:
    - Consistency between `pyscf.cc.rccsdt` (triangular T3 storage) and `pyscf.cc.rccsdt_highm` (full T3 storage).
    - Restarting RCCSDT from DIIS and checkpoint files.
    - Invariance of RCCSDT energy under orbital rotations.
'''
...
# Reference RCCSDT correlation energy
ref_e_corr = -0.2188784727114157

print("\n=== Consistency: triangular vs. full T3 storage ===\n")
# '''Check the consistency between RCCSDT in `pyscf.cc.rccsdt` and `pyscf.cc.rccsdt_highm`.'''
#
...
mycc5.verbose = 5
mycc5.diis_file = 'rccsdt_diis.h5'
mycc5.max_cycle = 5
mycc5.kernel()
print('RCCSDT e_corr            % .12f    Ref % .12f    Diff % .12e' % (
        mycc5.e_corr, ref_e_corr, mycc5.e_corr - ref_e_corr))
mycc5.chkfile = 'rccsdt.chk'
mycc5.dump_chk()
...
mycc8.max_cycle = 200
mycc8.kernel()
print('Rotated-MO RCCSDT e_corr % .12f    Ref % .12f    Diff % .12e' % (
