...
# As we don't have l2 in memory, hold tau temporarily in memory
    ...
    w3 = ccsd._cp(numpy.einsum('kc,jkbc->bj', fov, t2) * 2 + fov.T)
    w3 -= numpy.einsum('kc,kjbc->bj', fov, t2)
    w3 += reduce(numpy.dot, (t1.T, fov, t1.T))
    w4 = fov.copy()

    eris_ovvv = ccsd._cp(eris.ovvv)
    eris_ovvv = ccsd.unpack_tril(eris_ovvv.reshape(nov,-1))
    ...
    for i in range(nocc):
        ...
        lib.dot(eris_ovvv[i].reshape(-1,nvir).T,
                ccsd._cp(theta[i].reshape(nocc,-1)).T, 1, w3, 1)

    theta = ccsd._cp(theta.transpose(0,2,1,3))
    #:vkbca = numpy.einsum('jdca,kbjd->kbca', g2ovvv, theta)
    vkbca = lib.dot(ccsd._cp(theta.reshape(nov,-1)),
    ...
    eris_ovov = ccsd._cp(ccsd._cp(eris.ovov).transpose(0,2,1,3))
    ...
    wooov = ccsd._cp(tmp.reshape(-1,nvir,nocc,nocc).transpose(0,2,3,1))
    ...
    for i in range(nocc):
        t2tmp[i] = t2[i].transpose(2,0,1)
    tmp_ooov = ccsd._cp(-eris_ooov.transpose(2,0,1,3)).reshape(-1,nov)
    ...
    for i in range(nocc):
        lib.dot(ccsd._cp(theta[i].transpose(1,2,0)).reshape(-1,nvir).T,
    ...
    for i in range(nocc):
        lib.dot(ccsd._cp(eris_ooov[i].transpose(2,0,1)).reshape(-1,nocc),
    ...
    lib.dot(ccsd._cp(g2ovov.transpose(0,2,1,3)).reshape(-1,nov),
            ccsd._cp(t2.transpose(0,2,1,3).reshape(nov,-1).T), 1,
            vOVov.reshape(nov,-1), 1)
    lib.dot(eris_ovov.reshape(-1,nov),
            ccsd._cp(t2.transpose(0,3,1,2).reshape(nov,-1).T), -1,
            vOVov.reshape(nov,-1), 1)
    g2ovov = None
    ...
    for j in range(nocc):
        lib.dot(ccsd._cp(eris_ovov[j].transpose(1,0,2)).reshape(-1,nvir),
    ...
    lib.dot(ccsd._cp(ov2.transpose(0,2,1,3).reshape(nocc,-1)).T,
    ...
    wovvv += lib.dot(ccsd._cp(ov1.reshape(nocc,-1)).T,
                     t1).reshape(nvir,-1,nvir,nvir).transpose(1,0,3,2)
    ov1 = None

    woooo += ccsd._cp(eris.oooo).transpose(0,2,1,3)
    saved.woooo = woooo
    saved.wooov = wooov
    wooo = wooov = None
    ...
    lib.dot(ccsd._cp(tmp.reshape(-1,nocc)), t1, 1, wovvv.reshape(-1,nvir), 1)
    tmp = None
    lib.dot(ccsd._cp(g2ooov.transpose(0,2,1,3).reshape(nocc**2,-1)).T,
...
def update_amps(mycc, t1, t2, l1, l2, eris=None, saved=None):
    ...
    lib.dot(ccsd._cp(eris_ooov.transpose(0,2,1,3).reshape(nocc,-1)).T,
    ...
    m4 = tmp
    lib.dot(ccsd._cp(l2t1.reshape(nocc*nocc,-1)),
    ...
    saved_wOvOv = ccsd._cp(saved.wOvOv)
    tmp_ovov = ccsd._cp(saved.wOVov) * 2
    ...
    saved_woooo = ccsd._cp(saved.woooo)
...
def make_theta(t2, out=None):
    ...
    ccsd.libcc.CCmake_g0132(out.ctypes.data_as(ctypes.c_void_p),
                            t2.ctypes.data_as(ctypes.c_void_p),
                            t2.ctypes.data_as(ctypes.c_void_p),
                            shape.ctypes.data_as(ctypes.c_void_p),
                            ctypes.c_double(2), ctypes.c_double(-1))
    return out
